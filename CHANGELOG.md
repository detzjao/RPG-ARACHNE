...
## v48.2 — Correção da tela preta

- Corrige a incompatibilidade entre o frontend da v48 e o React 16 empacotado no projeto.
- Remove o uso de Hooks e `React.memo` que interrompiam a inicialização antes da primeira renderização.
- Mantém criação/edição de campanhas, gerenciamento de heróis/vilões e cenário da v48.
- Atualiza o identificador dos assets para `v48.2`, impedindo reutilização do JavaScript quebrado pelo navegador.
- Adiciona teste de regressão para impedir que APIs incompatíveis com a versão local do React voltem ao bundle.
