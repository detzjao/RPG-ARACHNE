...
A suíte atual inclui regressões do backend, iniciativa → grid, ameaças genéricas repetíveis, movimentação autoritativa, Mestre + dois jogadores, renovação de orçamento por turno, sincronização SSE e as etapas anteriores. A API/health reporta versão 33.7.0.
