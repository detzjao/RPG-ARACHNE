(() => {
  ...
  async function setChallengeTN(tn){const result=await request('/challenge/tn',{method:'PATCH',body:JSON.stringify({tn})});return result.data||{};}
  ...
  async function resetScenarioMovement(values){const result=await request('/scenario/movement/reset',{method:'PATCH',body:JSON.stringify(values||{})});return result.data;}
  ...
  window.ArachneAPI={CLIENT_ID,lookupCampaigns,lookupCampaign,getTemplates,getCharacters,createCampaign,join,setSession,clearSession,loadAll,saveStorageKey,saveMany,saveHero,deleteHero,saveVillain,deleteVillain,uploadAsset,searchCharacterImages,setCharacterImage,adjustResources,setChallengeTN,startCombat,nextCombatTurn,endCombat,updateCombatOrder,addInitiativeParticipant,removeInitiativeParticipant,clearInitiativeParticipants,rollInitiativeParticipant,moveScenarioPiece,resetScenarioMovement,startActionRoll,useActionEdge,finalizeActionRoll,applyAttackDamage,savePlayerNote,publishLiveRoll,health,probe,setApiBase,getApiBase,connectRealtime,disconnectRealtime,stateKeyFromStorage,get API_BASE(){return API_BASE;},get profile(){return profile;},get token(){return token;}};
})();
