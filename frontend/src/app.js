(() => {
  ...
  const INTEL = window.ArachneCharacterIntel || null;
  ...
  const TERRAIN_META = {
    floor:{label:'Piso',icon:'·',tone:'slate',elevation:0,land:true},labfloor:{label:'Piso técnico',icon:'⌁',tone:'steel',elevation:0,land:true},parking:{label:'Concreto',icon:'▦',tone:'concrete',elevation:0,land:true},roof:{label:'Telhado',icon:'▧',tone:'roof',elevation:0,land:true},wood:{label:'Madeira',icon:'≋',tone:'wood',elevation:0,land:true},stone:{label:'Pedra',icon:'◆',tone:'stone',elevation:0,land:true},grass:{label:'Solo / grama',icon:'✤',tone:'grass',elevation:0,land:true},road:{label:'Asfalto',icon:'━',tone:'road',elevation:0,land:true},
    elev1:{label:'Elevação +1',icon:'1',badge:'E1',tone:'elev1',elevation:1,land:true,raised:true,climbable:true},elev2:{label:'Elevação +2',icon:'2',badge:'E2',tone:'elev2',elevation:2,land:true,raised:true,climbable:true},
    ramp1:{label:'Rampa +1',icon:'↗',badge:'R1',tone:'ramp1',elevation:1,land:true,ramp:true,climbable:true},ramp2:{label:'Rampa +2',icon:'⇈',badge:'R2',tone:'ramp2',elevation:2,land:true,ramp:true,climbable:true},
    water:{label:'Água',icon:'≈',badge:'ÁGUA',tone:'water',elevation:0,water:true},gap:{label:'Vão / queda',icon:'↓',badge:'VÃO',tone:'gap',elevation:-1,gap:true}
  };
  const TERRAIN_TOOLS = new Set(['elev1','elev2','ramp1','ramp2','water','gap']);
  const OBSTACLE_META = {
    wall:{label:'Parede',icon:'🧱',block:true,group:'structure',help:'Bloqueia passagem.'},door:{label:'Porta',icon:'🚪',block:false,group:'structure',help:'Passagem entre paredes.'},fence:{label:'Grade',icon:'🚧',block:true,group:'structure',help:'Bloqueia passagem.'},pillar:{label:'Pilar',icon:'▮',block:true,group:'structure',help:'Estrutura sólida.'},cover:{label:'Cobertura',icon:'🛡',block:true,group:'structure',help:'Objeto grande usado como cobertura.'},
    crate:{label:'Caixa',icon:'📦',block:true,group:'objects',help:'Caixa que ocupa a casa.'},terminal:{label:'Terminal',icon:'🖥',block:true,group:'objects',help:'Computador ou painel.'},barrel:{label:'Barril',icon:'🛢',block:true,group:'objects',help:'Barril ou cilindro.'},rubble:{label:'Entulho',icon:'🪨',block:false,cost:2,group:'objects',help:'Pode atravessar, mas custa movimento extra.'},tree:{label:'Árvore',icon:'🌳',block:true,group:'objects',help:'Árvore que bloqueia a casa.'},rock:{label:'Rocha',icon:'🪨',block:true,group:'objects',help:'Rocha grande.'},car:{label:'Veículo',icon:'🚙',block:true,group:'objects',help:'Carro ou veículo parado.'},table:{label:'Mesa',icon:'▰',block:true,group:'objects',help:'Mesa que ocupa a casa.'},bed:{label:'Cama',icon:'🛏',block:true,group:'objects',help:'Cama ou maca.'},counter:{label:'Balcão',icon:'▤',block:true,group:'objects',help:'Balcão ou bancada.'},shelf:{label:'Estante',icon:'📚',block:true,group:'objects',help:'Estante ou arquivo.'},dock:{label:'Cais',icon:'═',block:false,group:'objects',help:'Passarela/cais transitável.'},log:{label:'Tora',icon:'🪵',block:true,group:'objects',help:'Tora caída.'},stump:{label:'Toco',icon:'🌲',block:true,group:'objects',help:'Toco de árvore.'},campfire:{label:'Fogueira',icon:'🔥',block:false,group:'objects',help:'Fogueira transitável.'},booth:{label:'Cabine',icon:'🏠',block:true,group:'objects',help:'Cabine pequena.'},statue:{label:'Estátua',icon:'🗿',block:true,group:'objects',help:'Estátua ou monumento.'}
  };
  const DECOR_META = {lane:{label:'Faixa no chão',icon:'━'},hazard:{label:'Área de risco',icon:'⚠'},lamp:{label:'Luz',icon:'💡'},stain:{label:'Marca no chão',icon:'●'},rug:{label:'Tapete',icon:'▧'},books:{label:'Livros',icon:'📚'},shrub:{label:'Arbusto',icon:'🌿'},pier:{label:'Passarela',icon:'═'},debris:{label:'Destroços',icon:'·'},tarp:{label:'Lona',icon:'▱'}};
  ...
  const NAV = [
    ['central','⌂','Central'], ['heroes','♙','Heróis'], ['npcs','◎','NPCs'], ['villains','☠','Vilões'], ['campaign','▣','Campanha'], ['scenario','▦','Cenário'], ['rules','?','Regras'], ['notes','✎','Anotações']
  ];
  const DEFAULT_DATA = {heroes:[],npcs:[],villains:[],campaignContent:{},challenge:{tn:14},scenario:{width:20,height:14,pieces:[],obstacles:{},terrain:{},movementSpent:{},turnMovement:{},minionVitals:{}},initiative:[],combat:{active:false,round:0,order:[]},actionHistory:[],dice:[],playerNotes:{},notesMaster:''};
  ...
  function reachableScenarioCellsUI(scenario,piece,mode,budget){
    const result=new Map(),limit=Number(budget||0);
    if(!piece||limit<=0)return result;
    const px=Number(piece.x),py=Number(piece.y);
    if(['jump','flight','glide','swingline','climb'].includes(mode)){
      const{w,h}=scenarioSize(scenario);
      for(let y=0;y<h;y++)for(let x=0;x<w;x++){
        if(x===px&&y===py)continue;
        if(!scenarioDirectAllowed(scenario,piece,x,y,mode))continue;
        const cost=scenarioDirectCost(scenario,piece,x,y);
        if(cost>0&&cost<=limit)result.set(scenarioKey(x,y),cost);
      }
      return result;
    }
    if(mode==='run'&&scenarioTerrain(scenario,px,py).water)return result;
    // Fila com cursor em vez de Array.shift(): evita realocar o array em cada passo.
    const queue=[[px,py,0]],best=new Map([[scenarioKey(px,py),0]]);
    let head=0;
    while(head<queue.length){
      const[x,y,cost]=queue[head++];
      for(const[nx,ny]of scenarioNeighbors(x,y)){
        if(!scenarioInBounds(scenario,nx,ny))continue;
        const ok=mode==='swim'?canScenarioSwim(scenario,x,y,nx,ny,piece):canScenarioRun(scenario,x,y,nx,ny,piece);
        if(!ok)continue;
        const nextCost=cost+scenarioCellCost(scenario,nx,ny,mode),key=scenarioKey(nx,ny);
        if(nextCost>limit)continue;
        if(best.has(key)&&best.get(key)<=nextCost)continue;
        best.set(key,nextCost);queue.push([nx,ny,nextCost]);
        if(!(nx===px&&ny===py))result.set(key,nextCost);
      }
    }
    return result;
  }
  ...
  function SheetModal({entity,kind,onClose}){
    ...
    const abilities=entity.abilities||{},damage=damageProfile(entity),defenses=entity.defenses||{},isHero=kind==='hero',isNpc=kind==='npc';
    ...
    return h('div',{className:'sheet-overlay fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-3',onMouseDown:e=>{if(e.target===e.currentTarget)onClose();}},h('div',{className:'sheet-modal w-full max-w-6xl overflow-auto rounded-2xl border border-white/10 bg-[#0b1017] shadow-2xl'},[
      h('div',{key:'hero',className:'sheet-hero grid border-b border-white/10'},[
        h('div',{key:'visual',className:'sheet-visual relative overflow-hidden bg-[#070a0f]',style:sheetPortraitStyle(entity)},[
          ...
          h('div',{key:'name',className:'sheet-visual-copy'},[h(TinyLabel,{key:'k',className:'sheet-visual-category text-[#ff7b85]'},isHero?'HERÓI':isNpc?'NPC':entity.tier||'AMEAÇA'),h('h2',{key:'n',className:'sheet-visual-name'},entity.n),h('p',{key:'r',className:'sheet-visual-realname'},entity.r||entity.role||'')])
        ]),
      ...
      h('div',{key:'body',className:'sheet-body grid gap-4 p-4 sm:p-6'},[
        h('div',{key:'left',className:'space-y-4'},[
          ...
          (entity.attacks||[]).length?h(Card,{key:'attacks',className:'p-4'},[h(TinyLabel,{key:'l',className:'text-[#ff7b85]'},'ATAQUES'),h('div',{key:'g',className:'mt-3 space-y-2'},entity.attacks.map((item,i)=>h('div',{key:i,className:'rounded-xl border border-[#ef3340]/20 bg-[#ef3340]/5 px-3 py-2 text-xs text-[#e6c6c9]'},item)))]):null,
          (entity.specialAbilities||[]).length?h(Card,{key:'skills',className:'p-4'},[h(TinyLabel,{key:'l'},'HABILIDADES'),h('div',{key:'g',className:'mt-3 space-y-2'},entity.specialAbilities.map((item,i)=>h('div',{key:i,className:'rounded-xl border border-white/10 bg-white/[.03] px-3 py-2 text-xs leading-5 text-[#c4ccd6]'},item)))]):null,
          (entity.powers||[]).length&&!((entity.attacks||[]).length||(entity.specialAbilities||[]).length)?h(Card,{key:'powers',className:'p-4'},[h(TinyLabel,{key:'l'},'PODERES'),h('div',{key:'g',className:'mt-3 flex flex-wrap gap-2'},chips(entity.powers,'power'))]):null
        ]),
        h('div',{key:'right',className:'space-y-4'},[
          ...
          h(Card,{key:'doc',className:'p-4'},[h(TinyLabel,{key:'l'},'DOCUMENTO'),(entity.sheet_pdf_url||entity.pdf)?h('div',{key:'docbody'},[h('a',{key:'pdf',href:entity.sheet_pdf_url||entity.pdf,target:'_blank',rel:'noopener',className:'mt-3 inline-flex min-h-11 w-full items-center justify-center rounded-xl border border-[#ef3340]/40 bg-[#ef3340]/10 px-4 text-xs font-black text-white hover:bg-[#ef3340]/15'},'VER FICHA COMPLETA EM PDF'),entity.pdf_page_start?h('p',{key:'origin',className:'mt-2 text-[10px] leading-4 text-[#75808e]'},`Origem: página ${entity.pdf_page_start}${entity.pdf_page_end&&entity.pdf_page_end!==entity.pdf_page_start?`–${entity.pdf_page_end}`:''} do arquivo PDF${entity.book_page_start?` · página ${entity.book_page_start}${entity.book_page_end&&entity.book_page_end!==entity.book_page_start?`–${entity.book_page_end}`:''} do livro`:''}.`):null]):h('p',{key:'p',className:'mt-2 text-xs text-[#75808e]'},entity.tier==='CAPANGA'?'Ficha completa integrada ao Arachne; não depende de PDF externo.':'Nenhum PDF vinculado a esta ficha.')])
        ])
      ])
    ]));
  }
  ...
  function catalogCategory(item){
    if(INTEL?.category)return INTEL.category(item);
    const tier=String(item?.tier||'').toUpperCase(),role=String(item?.role||'').toLocaleLowerCase('pt-BR');
    if(item?.alignment==='antihero'||tier==='ANTI-HERÓI'||role.includes('anti-herói'))return'antihero';
    if(item?.generic===true||tier==='CAPANGA'||tier==='LACAIO')return'minion';
    return item?.libraryKind==='hero'?'hero':'villain';
  }
  function catalogCategoryLabel(category,singular=false){
    const labels=singular?{all:'Todos',hero:'Herói',antihero:'Anti-herói',npc:'NPC',villain:'Vilão',minion:'Capanga'}:{all:'Todos',hero:'Heróis',antihero:'Anti-heróis',npc:'NPCs',villain:'Vilões',minion:'Capangas'};
    return labels[category]||category;
  }
  function rosterSideForCategory(category){return category==='hero'||category==='antihero'?'hero':'villain';}
  function categoryTone(category){return category==='hero'?'border-white/10 text-[#7f8997]':category==='npc'?'border-sky-700/50 text-sky-300':category==='antihero'?'border-amber-700/50 text-amber-300':category==='minion'?'border-violet-700/50 text-violet-300':'border-[#ef3340]/40 text-[#ff7b85]';}
  function filteredCategoryItems(items,category){return(items||[]).filter(item=>catalogCategory(item)===category);}
  function allSelectedCharacterIds(state){return[...(state?.createHeroIds||[]),...(state?.createNpcIds||[]),...(state?.createVillainIds||[])];}
  ...
  const characterSourceId=item=>String(item?.sourceCharacterId||item?.baseId||item?.id||'');
  const isMinionCharacter=item=>Boolean(item&&(item.generic===true||item.type==='minion'||['CAPANGA','LACAIO'].includes(String(item.tier||'').toUpperCase())||catalogCategory(item)==='minion'));

  function blankCharacter(kind='hero'){
    const id=`${kind}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,6)}`;
    return {id,n:kind==='hero'?'Novo Herói':kind==='npc'?'Novo NPC':'Novo Vilão',r:'',rank:4,tier:kind==='hero'?'HERÓI':kind==='npc'?'NPC':'AMEAÇA',image:'',pdf:'',role:'',origin:'',occupation:'',teams:'',base:'',karma:kind==='hero'?4:'—',healthDR:'—',focusDR:'—',initiative:'+0',maxHealth:90,currentHealth:90,maxFocus:90,currentFocus:90,movement:{run:5,climb:3,swim:3,jump:3},speed:'Correr 5 · Escalar 3 · Nadar 3 · Pular 3',abilities:{Melee:0,Agility:0,Resilience:0,Vigilance:0,Ego:0,Logic:0},damageMultipliers:{Melee:4,Agility:4,Ego:4,Logic:4},traits:[],tags:kind==='hero'?['Heroic']:kind==='npc'?[]:['Villainous'],powers:[]};
  }
  function CharacterPickerModal({kind,items,existing,onAdd,onCreate,onClose,search='',onSearch,category='',onCategory,group='',onGroup,selectedIds=[],campaignItems=[],campaignRosters={},focusId='',templates=[]}){
    const allowed=['all','hero','villain','minion'];
    const activeCategory=allowed.includes(category)?category:'all';
    ...
    let filtered=ranked.filter(row=>activeCategory==='all'||(activeCategory==='hero'&&['hero','antihero'].includes(catalogCategory(row.item)))||catalogCategory(row.item)===activeCategory)
    ...
    const title=kind==='hero'?'Adicionar Heróis':kind==='npc'?'Adicionar NPCs controlados pelo Mestre':'Adicionar Vilões / Capangas';
    return h('div',{className:'character-picker-overlay fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-3',onMouseDown:e=>{if(e.target===e.currentTarget)onClose();}},h('div',{className:'character-picker-modal w-full max-w-6xl overflow-hidden rounded-2xl border border-white/10 bg-[#0d1118] shadow-2xl'},[
      h('div',{key:'h',className:'character-picker-header flex flex-wrap items-center justify-between gap-3 border-b border-white/10 p-4 sm:p-5'},[h('div',{key:'t'},[h(TinyLabel,{key:'l'},'CATÁLOGO INTELIGENTE'),h('h2',{key:'n',className:'mt-1 text-xl font-black'},title),h('p',{key:'p',className:'mt-1 text-xs text-[#7e8997]'},`A biblioteca é compartilhada, mas personagens comuns ocupam um papel por vez. Capangas são a exceção e podem coexistir em mais de um papel. Aqui ele será adicionado à campanha como ${kind==='hero'?'Herói jogável':kind==='npc'?'NPC controlado pelo Mestre':'Vilão/ameaça'}.`)]),h(Button,{key:'x',onClick:onClose,className:'px-3'},'×')]),
      h('div',{key:'filters',className:'character-picker-filters space-y-3 border-b border-white/5 p-4'},[
        h('div',{key:'cats',className:'flex flex-wrap gap-2'},allowed.map(cat=>h('button',{key:cat,type:'button',onClick:()=>onCategory&&onCategory(cat),className:cx('rounded-xl border px-3 py-2 text-xs font-black transition',activeCategory===cat?'border-[#ef3340] bg-[#ef3340]/10 text-white':'border-white/10 bg-black/20 text-[#7f8997] hover:border-white/20')},catalogCategoryLabel(cat)))),
        h('div',{key:'tools',className:'flex flex-wrap gap-2'},[
          h('input',{key:'i',value:search,onChange:e=>onSearch&&onSearch(e.target.value),placeholder:'Buscar nome, identidade, função ou equipe...',autoFocus:true,className:'min-h-11 min-w-[220px] flex-1 rounded-xl border border-white/10 bg-black/30 px-4 text-sm outline-none focus:border-[#ef3340]/60'}),
          h('select',{key:'group',value:group||'',onChange:e=>onGroup&&onGroup(e.target.value),className:'min-h-11 min-w-[220px] rounded-xl border border-white/10 bg-[#111720] px-3 text-xs font-bold outline-none'},[h('option',{key:'all',value:''},'TODOS OS GRUPOS'),...groups.map(g=>h('option',{key:g.id,value:g.id},`${g.label} (${g.count})`))]),
          h(Button,{key:'new',primary:true,onClick:onCreate},'CRIAR DO ZERO')
        ]),
      ...
      h('div',{key:'g',className:'catalog-grid-scroll character-scroll-area character-picker-list grid gap-2 p-4 sm:grid-cols-2 lg:grid-cols-3'},filtered.length?filtered.map(row=>{const item=row.item,sourceId=characterSourceId(item),sourceIsMinion=isMinionCharacter(item),isRepeatableMinion=kind==='villain'&&activeCategory==='minion',instanceCount=isRepeatableMinion?(campaignItems||[]).filter(x=>characterSourceId(x)===sourceId).length:0,heroMatch=(campaignRosters.heroes||[]).find(x=>characterSourceId(x)===sourceId),npcMatch=(campaignRosters.npcs||[]).find(x=>characterSourceId(x)===sourceId),villainMatch=(campaignRosters.villains||[]).find(x=>characterSourceId(x)===sourceId),blockedRole=!sourceIsMinion?(kind==='npc'?(heroMatch?'HERÓI':villainMatch?'VILÃO':''):(kind==='hero'||kind==='villain')&&npcMatch?'NPC':''):'',used=!isRepeatableMinion&&existing.has(item.id),disabled=used||Boolean(blockedRole),reasons=row.reasons||[],officialGroups=INTEL?.officialGroups?INTEL.officialGroups(item):String(item.teams||'').split(',').map(x=>x.trim()).filter(Boolean);return h('button',{key:item.id,type:'button',disabled,onClick:()=>!disabled&&onAdd(item.id,activeCategory),className:cx('relative flex items-center gap-3 rounded-xl border p-3 text-left transition',disabled?'cursor-not-allowed border-white/5 bg-white/[.02] opacity-45':row.score>0?'border-[#ef3340]/25 bg-[#171d27] hover:border-[#ef3340]/55':'border-white/10 bg-[#111720] hover:border-[#ef3340]/55')},[
        ...
        h(Portrait,{key:'p',entity:item,size:'sm'}),h('div',{key:'c',className:'min-w-0 flex-1'},[
          ...
          blockedRole?h('span',{key:'u',className:'mt-1 block text-[8px] font-black text-amber-300'},`JÁ COMO ${blockedRole} · REMOVA PRIMEIRO`):isRepeatableMinion?h('span',{key:'u',className:'mt-1 block text-[8px] font-black text-violet-300'},instanceCount?`${instanceCount} NA CAMPANHA · PODE REPETIR`:'PODE REPETIR'):used?h('span',{key:'u',className:'mt-1 block text-[8px] font-black text-emerald-400'},'JÁ NA CAMPANHA'):null
        ])
    ...
    render(){
      ...
      return h('div',{className:'fixed inset-0 z-[55] flex items-center justify-center bg-black/85 p-3',onMouseDown:e=>{if(e.target===e.currentTarget)onClose();}},h('div',{className:'max-h-[94vh] w-full max-w-5xl overflow-auto rounded-2xl border border-white/10 bg-[#0d1118] p-4 shadow-2xl sm:p-6'},[
        ...
        h('div',{key:'base',className:'mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4'},[
          ['Nome','n',draft.n,'text'],['Nome civil / descrição','r',draft.r,'text'],['Rank','rank',draft.rank,'number'],[kind==='hero'?'Função':kind==='npc'?'Papel':'Tipo','tier',draft.tier||'','text']
  ...
  class CampaignEditor extends React.Component {
    ...
    render(){
      const {campaign,heroes,npcs,villains,onSave,onCancel,onAddCategory,onEditCharacter,onRemoveCharacter,onUploadPdf,saving=false}=this.props,draft=this.state.draft;
      ...
      const heroRows=(heroes||[]).map(entity=>({entity,side:'hero'})),npcRows=(npcs||[]).map(entity=>({entity,side:'npc'})),villainRows=(villains||[]).map(entity=>({entity,side:'villain'}));
      const roster=category=>{let items=[];if(category==='hero')items=heroRows;else if(category==='npc')items=npcRows;else if(category==='minion')items=villainRows.filter(row=>catalogCategory(row.entity)==='minion');else items=villainRows.filter(row=>catalogCategory(row.entity)!=='minion');const roleLabel=side=>side==='hero'?'JOGÁVEL':side==='npc'?'NPC · MESTRE':'AMEAÇA';return h(Card,{className:'p-4'},[h('div',{key:'h',className:'flex items-center justify-between gap-2'},[h('div',{key:'t'},[h(TinyLabel,{key:'l'},catalogCategoryLabel(category).toUpperCase()),h('b',{key:'n',className:'mt-1 block text-sm'},`${items.length} na campanha`)]),h(Button,{key:'a',onClick:()=>onAddCategory(category),className:'min-h-9 py-1'},'+ ADICIONAR')]),h('div',{key:'l',className:'campaign-editor-roster-scroll character-scroll-area mt-3 space-y-2'},items.length?items.map(({entity,side})=>h('div',{key:`${side}-${entity.id}`,className:'flex items-center gap-3 rounded-xl border border-white/8 bg-black/15 p-2'},[h(Portrait,{key:'p',entity,size:'sm'}),h('div',{key:'n',className:'min-w-0 flex-1'},[h('b',{key:'b',className:'block truncate text-xs'},entity.n),h('small',{key:'s',className:'block truncate text-[9px] text-[#7c8795]'},`${entity.r||`Rank ${entity.rank||4}`} · ${roleLabel(side)}`)]),h(Button,{key:'e',onClick:()=>onEditCharacter(side,entity),className:'min-h-8 px-2 py-1'},'EDITAR'),h(Button,{key:'r',danger:true,onClick:()=>onRemoveCharacter(side,entity),className:'min-h-8 px-2 py-1'},'×')])):h('div',{className:'rounded-xl border border-dashed border-white/10 p-5 text-center text-xs text-[#75808e]'},`Nenhum ${catalogCategoryLabel(category,true).toLocaleLowerCase('pt-BR')} adicionado.`))]);};
      return h('div',{className:'space-y-4'},[
        ...
        h('div',{key:'grid',className:'grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(340px,.8fr)]'},[
          ...
          h('div',{key:'rosters',className:'grid gap-4'},[roster('hero'),roster('npc'),roster('villain'),roster('minion')])
        ]),
  ...
  function CharacterStrip({items,selectedKey,onSelect,mode='villain'}){
    return h('div',{className:'scrollbar-thin flex gap-2 overflow-x-auto pb-2'},items.map(item=>h('button',{key:item.key,onClick:()=>onSelect(item.key),className:cx('min-w-[158px] rounded-xl border bg-[#111720] p-3 text-left transition',selectedKey===item.key?'border-[#ef3340] bg-[#2a1117]':'border-white/10 hover:border-white/20')},[
      ...
      h('small',{key:'k',className:'text-[9px] font-bold text-[#788291]'},item.kind==='hero'?'HERÓI':item.kind==='npc'?'NPC':item.kind==='other'?'CAPANGA':mode==='hero'?'HERÓI':'VILÃO')
    ])));
  }
  ...
  function scenarioToolInfo(id,current){
    if(id==='select')return{icon:'✥',label:'Selecionar / mover',help:'Clique em uma peça para ver o alcance e depois clique em uma casa verde.',group:'main'};
    if(id==='erase')return{icon:'⌫',label:'Apagar',help:'Remove terreno especial, objeto, detalhe ou peça da casa.',group:'main'};
    if(id==='base')return{icon:'▦',label:'Piso normal',help:`Volta a casa para ${TERRAIN_META[current?.baseTerrain]?.label||'o piso do ambiente'}.`,group:'terrain'};
    if(TERRAIN_META[id])return{icon:TERRAIN_META[id].icon||'▦',label:TERRAIN_META[id].label,help:id==='water'?'Área de água: exige movimento compatível.':id==='gap'?'Vão/queda: não é piso normal.':'Altera a altura ou o tipo de terreno.',group:'terrain'};
    if(OBSTACLE_META[id])return{icon:OBSTACLE_META[id].icon||'■',label:OBSTACLE_META[id].label,help:OBSTACLE_META[id].help||(OBSTACLE_META[id].block?'Bloqueia a passagem.':'Pode ser atravessado.'),group:OBSTACLE_META[id].group||'objects'};
    if(String(id).startsWith('decor-')){const key=String(id).slice(6),meta=DECOR_META[key];return{icon:meta?.icon||'·',label:meta?.label||'Detalhe',help:'Detalhe visual. Não bloqueia o movimento.',group:'decor'};}
    return{icon:'?',label:String(id||'Ferramenta'),help:'Clique no mapa para usar esta ferramenta.',group:'main'};
  }

  function ScenarioLegend({scenario}){
    const current=normalizedScenario(scenario),terrainIds=[...new Set(Object.values(current.terrain||{}))],obstacleIds=[...new Set(Object.values(current.obstacles||{}))],decorIds=[...new Set(Object.values(current.decor||{}))];
    const terrainRows=terrainIds.map(id=>({id,icon:TERRAIN_META[id]?.icon||'▦',label:TERRAIN_META[id]?.label||id,note:TERRAIN_META[id]?.water?'água':TERRAIN_META[id]?.gap?'não transitável como piso':TERRAIN_META[id]?.raised||TERRAIN_META[id]?.ramp?'altura diferente':'terreno'}));
    const obstacleRows=obstacleIds.map(id=>({id,icon:OBSTACLE_META[id]?.icon||'■',label:OBSTACLE_META[id]?.label||id,note:OBSTACLE_META[id]?.block?'bloqueia':'transitável'}));
    const decorRows=decorIds.map(id=>({id,icon:DECOR_META[id]?.icon||'·',label:DECOR_META[id]?.label||id,note:'visual'}));
    const rows=[...terrainRows,...obstacleRows,...decorRows];
    const shown=rows.length?rows:[{id:'piece',icon:'●',label:'Personagem',note:'peça no mapa'},{id:'move',icon:'🟩',label:'Casa verde',note:'destino disponível'}];
    return h('div',{className:'scenario-legend border-t border-white/5 bg-[#0a0e14] px-4 py-3'},[
      h('div',{key:'title',className:'mb-2 flex flex-wrap items-center justify-between gap-2'},[h(TinyLabel,{key:'l'},'LEGENDA DO MAPA'),h('span',{key:'h',className:'text-[10px] text-[#6f7b8b]'},'A legenda mostra os elementos usados neste mapa.')]),
      h('div',{key:'rows',className:'flex flex-wrap gap-2'},shown.map(row=>h('span',{key:`${row.id}-${row.label}`,className:'legend-chip'},[h('span',{key:'i',className:'legend-icon'},row.icon),h('b',{key:'l'},row.label),h('small',{key:'n'},row.note)])))
    ]);
  }

  function Board({scenario,heroes,npcs,villains,role,heroId,selected,onSelect,onMove,zoom,onZoom,onReset,onResetPiece,moveMode='run',onMoveMode,editor=false,editorTool='select',onEditCell,onRemovePiece,title=true,moving=false}){
    ...
    const charById=(()=>{
      const map=new Map();
      for(const entity of [...(heroes||[]),...(npcs||[]),...(villains||[])])map.set(String(entity.id),entity);
    ...
    for(let y=0;y<height;y++)for(let x=0;x<width;x++){
      ...
      const details=[`Casa ${x+1},${y+1}`,piece&&(piece.name||entity?.n),obMeta?.label,terrainMeta.label].filter(Boolean).join(' · ');
      const click=()=>{
        if(moving)return;
      ...
      cells.push(h('button',{key,type:'button',onClick:click,disabled:moving&&isReachable,className:cx('board-cell',`terrain-${terrain.type}`,ob&&`obstacle-${ob}`,ob&&'has-obstacle',piece&&'has-piece',effectiveSelected&&piece?.id===effectiveSelected&&'selected-piece',isReachable&&'reachable-cell',editor&&editorTool!=='select'&&'editor-cell'),'data-x':x,'data-y':y,'aria-label':isReachable?`${details} · destino disponível`:details},[
        terrainMeta.badge?h('span',{key:'tb',className:'terrain-badge'},terrainMeta.badge):null,
        decor?h('span',{key:'d',className:'decor-mark',title:decorMeta?.label||decor},decorMeta?.icon||'·'):null,
        ob?h('span',{key:'o',className:'obstacle-mark',title:obMeta?.label||ob},obMeta?.icon||'■'):null,
    ...
    const movementPanel=selectedPiece&&movement?h('div',{className:'movement-panel mx-4 mt-3 rounded-2xl p-3 sm:p-4'},[
      h('div',{key:'top',className:'flex flex-wrap items-center justify-between gap-3'},[
        h('div',{key:'actor',className:'flex min-w-0 items-center gap-3'},[h(Portrait,{key:'p',entity:selectedEntity||{n:selectedPiece.name,image:selectedPiece.image},size:'sm'}),h('div',{key:'n',className:'min-w-0'},[h(TinyLabel,{key:'l'},role==='player'?'SEU PERSONAGEM':'PEÇA SELECIONADA'),h('b',{key:'b',className:'mt-1 block truncate text-sm'},selectedPiece.name||selectedEntity?.n||'Personagem')])]),
        h('div',{key:'remaining',className:'rounded-xl border border-emerald-500/20 bg-black/20 px-3 py-2 text-right'},[h(TinyLabel,{key:'l'},'MOVIMENTO RESTANTE'),h('strong',{key:'v',className:'mt-1 block text-xl text-emerald-300'},`${movement.remaining} / ${movement.max}`)])
      ]),
      h('div',{key:'modes',className:'mt-3 flex flex-wrap gap-2'},movement.valid.map(([mode,value])=>h('button',{key:mode,type:'button',disabled:Boolean(movement.locked&&movement.locked.mode!==mode)||moving,onClick:()=>onMoveMode&&onMoveMode(mode),className:cx('movement-mode-btn',movement.mode===mode&&'active','disabled:opacity-30')},[h('b',{key:'l'},MOVE_META[mode]?.label||mode),h('span',{key:'v'},value)]))),
      h('div',{key:'bar',className:'mt-3 h-2 overflow-hidden rounded-full bg-white/5'},h('i',{className:'block h-full rounded-full bg-emerald-500',style:{width:`${movement.max?Math.max(0,Math.min(100,movement.remaining/movement.max*100)):0}%`}})),
      h('div',{key:'actions',className:'mt-3 flex flex-wrap items-center justify-between gap-2'},[
        h('span',{key:'hint',className:'text-[11px] text-[#8f99a8]'},moving?'Movendo peça…':movement.remaining>0?'As casas verdes são os destinos disponíveis. Clique em uma delas para mover.':'O movimento desta peça acabou nesta rodada.'),
        h('div',{key:'buttons',className:'flex gap-2'},[onResetPiece?h(Button,{key:'reset',disabled:moving,onClick:()=>onResetPiece(selectedPiece.id),className:'min-h-9 py-1'},'REINICIAR MOVIMENTO'):null,editor&&role==='master'&&onRemovePiece?h(Button,{key:'remove',danger:true,disabled:moving,onClick:()=>onRemovePiece(selectedPiece.id),className:'min-h-9 py-1'},'REMOVER PEÇA'):null])
      ])
    ]):h('div',{className:'mx-4 mt-3 rounded-xl border border-dashed border-white/10 bg-white/[.02] px-3 py-3 text-xs text-[#8792a1]'},role==='player'?(ownPiece?'Sua peça está no mapa. Selecione um modo de movimento para ver as casas disponíveis.':'Seu personagem ainda não foi colocado no cenário.'):(editorTool==='select'?'Clique em uma peça para selecioná-la. As casas verdes mostram o alcance disponível.':`Você está editando o mapa com “${scenarioToolInfo(editorTool,current).label}”. Clique nas casas para aplicar.`));
    return h(Card,{className:'overflow-hidden scenario-board-card'},[
      title?h('div',{key:'h',className:'flex flex-wrap items-center justify-between gap-3 p-4 sm:p-5'},[
        h('div',{key:'t'},[h(TinyLabel,{key:'s'},editor?'MAPA DA CENA':'CENÁRIO'),h('h2',{key:'h',className:'mt-1 text-xl font-black'},editor?'Visualização do mapa':role==='master'?'Controle das peças':'Seu movimento'),h('div',{key:'chips',className:'mt-2 flex flex-wrap gap-2 text-[10px] text-[#7e8998]'},[h('span',{key:'a',className:'board-help-chip'},'🟩 área verde = alcance'),h('span',{key:'b',className:'board-help-chip'},'🔴 contorno = peça selecionada'),h('span',{key:'c',className:'board-help-chip'},'ícones = objetos')])]),
        h('div',{key:'c',className:'flex items-center gap-2'},[h(Button,{key:'m',onClick:()=>onZoom(Math.max(.35,zoom-.08)),className:'px-3'},'−'),h('span',{key:'z',className:'w-14 text-center text-xs font-black'},`${Math.round(zoom*100)}%`),h(Button,{key:'p',onClick:()=>onZoom(Math.min(1.4,zoom+.08)),className:'px-3'},'+'),onReset?h(Button,{key:'r',disabled:moving,onClick:onReset},role==='master'?'NOVA RODADA':'REINICIAR'):null])
      ]):null,
      movementPanel,
      h('div',{key:'scroll',className:'board-scroll mt-3 overflow-auto bg-[#05080c] p-3 sm:p-4'},h('div',{className:'board-grid',style:{gridTemplateColumns:`repeat(${width}, ${cellSize}px)`,gridTemplateRows:`repeat(${height}, ${cellSize}px)`,transform:`scale(${zoom})`,transformOrigin:'top left',width:`${width*cellSize}px`,height:`${height*cellSize}px`}},cells)),
      h(ScenarioLegend,{key:'legend',scenario:current})
    ]);
  }
  ...
  function ScenarioBuilder({scenario,heroes,npcs,villains,selected,onSelect,onMove,zoom,onZoom,onReset,onResetPiece,moveMode,onMoveMode,tool,onTool,preset,onPreset,onApplyPreset,onGenerate,onClear,onResize,onEnvironment,pieceChoice,onPieceChoice,qty,onQty,onAddPieces,onEditCell,onRemovePiece,group='terrain',onGroup,moving=false}){
    const current=normalizedScenario(scenario);
    const actorOptions=[...(heroes||[]).map(e=>({key:`hero|${e.id}`,label:`Herói · ${e.n}`})),...(npcs||[]).map(e=>({key:`npc|${e.id}`,label:`NPC · ${e.n}`})),...(villains||[]).map(e=>({key:`villain|${e.id}`,label:`Ameaça · ${e.n}`}))];
    ...
    const toolTile=(id,meta)=>h('button',{key:id,type:'button',onClick:()=>onTool(id),className:cx('scenario-tool-tile',tool===id&&'active')},[h('span',{key:'i',className:'scenario-tool-icon'},meta.icon),h('span',{key:'c',className:'min-w-0'},[h('b',{key:'l'},meta.label),h('small',{key:'h'},meta.help)])]);
    return h('div',{className:'space-y-4'},[
      h('section',{key:'head'},[h(TinyLabel,{key:'s'},'MONTADOR TÁTICO'),h('h1',{key:'h',className:'mt-1 text-3xl font-black'},'Montador de cenário'),h('p',{key:'p',className:'mt-1 max-w-3xl text-sm leading-6 text-[#8b95a3]'},'Crie um mapa simples para a sessão. Você não precisa decorar tudo: escolha um modelo, coloque paredes e objetos importantes e adicione as peças.')]),
      h('div',{key:'steps',className:'grid gap-2 sm:grid-cols-3'},[
        ['1','Escolha a base','Use um modelo pronto ou gere um mapa.'],['2','Desenhe o necessário','Paredes, portas, água e objetos importantes.'],['3','Coloque os personagens','Depois mova as peças diretamente no grid.']
      ].map(x=>h('div',{key:x[0],className:'scenario-step'},[h('span',{key:'n'},x[0]),h('div',{key:'c'},[h('b',{key:'b'},x[1]),h('small',{key:'s'},x[2])])]))),
      h(Card,{key:'quick',className:'p-4 sm:p-5'},[
        h('div',{key:'title',className:'mb-4 flex items-center justify-between gap-3'},[h('div',{key:'t'},[h(TinyLabel,{key:'l'},'COMECE RÁPIDO'),h('h2',{key:'h',className:'mt-1 text-lg font-black'},'Base do mapa')]),h('span',{key:'e',className:'rounded-full border border-white/10 px-3 py-1 text-[10px] text-[#7d8795]'},`${current.width} × ${current.height} casas`)]),
        h('div',{key:'row1',className:'grid gap-3 lg:grid-cols-[1.2fr_1fr_1fr_auto_auto]'},[
          h('label',{key:'preset',className:'grid gap-1'},[h(TinyLabel,{key:'l'},'MODELO PRONTO'),h('select',{key:'s',value:preset,onChange:e=>onPreset(e.target.value),className:'scenario-select'},Object.entries(SCENARIO_PRESETS).map(([id,meta])=>h('option',{key:id,value:id},meta.label)))]),
          h('label',{key:'env',className:'grid gap-1'},[h(TinyLabel,{key:'l'},'TIPO DE LOCAL'),h('select',{key:'s',value:current.environment,onChange:e=>onEnvironment(e.target.value),className:'scenario-select'},Object.entries(ENVIRONMENTS).map(([id,meta])=>h('option',{key:id,value:id},meta.label)))]),
          h('label',{key:'size',className:'grid gap-1'},[h(TinyLabel,{key:'l'},'TAMANHO'),h('select',{key:'s',value:`${current.width}x${current.height}`,onChange:e=>onResize(e.target.value),className:'scenario-select'},['10x8','12x9','16x12','18x12','18x14','20x14','22x14','22x16','24x16','24x18','30x20'].map(v=>h('option',{key:v,value:v},v.replace('x',' × '))))]),
      ...
      h('div',{key:'work',className:'scenario-work-grid grid gap-4'},[
        ...
        h(Card,{key:'pieces',className:'p-4 sm:p-5'},[
          h(TinyLabel,{key:'l'},'ADICIONAR PEÇAS'),h('h2',{key:'h',className:'mt-1 text-lg font-black'},'Personagens da cena'),h('p',{key:'p',className:'mt-1 text-xs leading-5 text-[#7f8998]'},'Escolha quem entra na cena. As peças aparecem automaticamente em casas livres.'),
          h('label',{key:'actor',className:'mt-4 grid gap-1'},[h(TinyLabel,{key:'l'},'PERSONAGEM'),h('select',{key:'a',value:pieceChoice||actorOptions[0]?.key||'',onChange:e=>onPieceChoice(e.target.value),className:'scenario-select'},actorOptions.map(o=>h('option',{key:o.key,value:o.key},o.label)))]),
          h('label',{key:'qty',className:'mt-3 grid gap-1'},[h(TinyLabel,{key:'l'},'QUANTIDADE'),h('input',{key:'q',type:'number',min:1,max:20,value:qty,onChange:e=>onQty(clamp(e.target.value,1,20)),className:'scenario-input'})]),
          h(Button,{key:'add',primary:true,onClick:onAddPieces,className:'mt-3 w-full'},'ADICIONAR AO MAPA'),
          h('div',{key:'tips',className:'mt-4 rounded-xl border border-white/10 bg-black/20 p-3 text-[11px] leading-5 text-[#8a94a2]'},[h('b',{key:'b',className:'block text-white'},'Durante a sessão'),h('span',{key:'s'},'Use “Selecionar / mover”, clique numa peça e depois numa casa verde.')]),
          h(Button,{key:'clear',danger:true,onClick:onClear,className:'mt-4 w-full'},'LIMPAR CENÁRIO')
        ])
      ]),
      h(DeferredBoard,{key:'board',scenario:current,heroes,npcs,villains,role:'master',selected,onSelect,onMove,zoom,onZoom,onReset,onResetPiece,moveMode,onMoveMode,editor:true,editorTool:tool,onEditCell,onRemovePiece,moving})
    ]);
  }
  ...
  class ArachneApp extends React.Component {
    constructor(props){
      super(props);
      this.state={screen:'loading',campaigns:[],templates:[],characters:{heroes:[],villains:[]},selectedCampaign:null,accessMode:'',joinCode:'',masterPassword:'',createOpen:false,createMode:'blank',createTemplate:'',createName:'',createPassword:'',createHeroIds:[],createNpcIds:[],createVillainIds:[],createRosterKind:'',createRosterSide:'',createRosterSearch:'',createRosterGroup:'',createFocusCharacterId:'',campaignEditOpen:false,campaignSaving:false,rosterPicker:'',rosterSearch:'',rosterCategory:'',rosterGroup:'',rosterFocusCharacterId:'',characterEditor:null,profile:null,data:{...DEFAULT_DATA},page:'central',navOpen:false,masterActorView:'villain',actorKey:'',category:'combat',power:'',roll:null,rollAnimating:false,initiativeRoll:null,initiativeAnimating:false,tn:14,edge:0,trouble:0,selectedPiece:null,zoom:.72,moveMode:'run',scenarioTool:'select',scenarioGroup:'terrain',scenarioPreset:'empty',scenarioPieceChoice:'',scenarioQty:1,scenarioMoveBusy:false,sheet:null,status:'offline',toast:'',error:''};
    ...
    componentWillUnmount(){ API.disconnectRealtime(); clearTimeout(this.toastTimer); clearTimeout(this.tnTimer); clearTimeout(this.scenarioSaveTimer); }
    ...
    connectRealtime(){ API.connectRealtime({onStatus:status=>this.setState({status}),onState:event=>{if(!event?.key)return;if(event.key==='scenario'&&this.scenarioSavePending)return;const value=event.key==='scenario'?normalizedScenario(event.value||{}):event.value;this.setState(prev=>({data:{...prev.data,[event.key]:value},tn:event.key==='challenge'?clamp(value?.tn||prev.tn,1,99):prev.tn}),()=>this.ensureActor());}}); }
    ensureActor(){
    ...
    masterActors(){
      const d=this.state.data,solo=d.campaignContent?.mode==='solo',view=this.state.masterActorView||'villain'; const out=[];
      if(solo)(d.heroes||[]).forEach(e=>out.push({key:`hero|${e.id}`,kind:'hero',entity:e,label:e.n}));
      const list=view==='npc'?(d.npcs||[]):(d.villains||[]);
      list.forEach(e=>out.push({key:`${view}|${e.id}`,kind:view,entity:e,label:e.n}));
      return out;
    }
    currentEntity(){
    ...
    creationLibrary(kind='hero'){return uniqueCharacterLibrary(this.state.characters,this.state.templates,kind);}
    chooseCreationBase(templateId=''){
      const template=this.state.templates.find(t=>t.id===templateId);
      if(template)this.setState({createMode:'template',createTemplate:template.id,createName:template.name,createHeroIds:(template.heroes||[]).map(x=>x.id),createNpcIds:(template.npcs||[]).map(x=>x.id),createVillainIds:(template.villains||[]).map(x=>x.id),createRosterKind:'',createRosterSide:'',createRosterSearch:'',createRosterGroup:'',createFocusCharacterId:(template.heroes||[])[0]?.id||(template.villains||[])[0]?.id||''});
      else this.setState({createMode:'blank',createTemplate:'',createName:'Nova Campanha',createHeroIds:[],createNpcIds:[],createVillainIds:[],createRosterKind:'',createRosterSide:'',createRosterSearch:'',createRosterGroup:'',createFocusCharacterId:''});
    }
    toggleCreationCharacter(kind,id){
      const target=kind==='npc'?'createNpcIds':kind==='hero'?'createHeroIds':'createVillainIds';
      const source=this.creationLibrary('hero').find(item=>String(item.id)===String(id));
      const sourceIsMinion=isMinionCharacter(source);
      const current=this.state;
      const removing=(current[target]||[]).includes(id);
      if(!removing&&!sourceIsMinion){
        if(kind==='npc'&&((current.createHeroIds||[]).includes(id)||(current.createVillainIds||[]).includes(id))){this.toast('Este personagem já está selecionado como Herói ou Vilão. Remova-o desse papel antes de usá-lo como NPC.');return;}
        if((kind==='hero'||kind==='villain')&&(current.createNpcIds||[]).includes(id)){this.toast('Este personagem já está selecionado como NPC. Remova-o dos NPCs antes de mudar o papel.');return;}
      }
      this.setState(prev=>{
        const next={createHeroIds:[...(prev.createHeroIds||[])],createNpcIds:[...(prev.createNpcIds||[])],createVillainIds:[...(prev.createVillainIds||[])]};
        if(removing)next[target]=next[target].filter(x=>x!==id);
        else if(sourceIsMinion){if(!next[target].includes(id))next[target].push(id);}
        else if(kind==='npc'){next[target].push(id);}
        else{const opposite=kind==='hero'?'createVillainIds':'createHeroIds';next[opposite]=next[opposite].filter(x=>x!==id);next[target].push(id);}
        return{...next,createFocusCharacterId:removing&&prev.createFocusCharacterId===id?'':id};
      });
    }
    async createCampaign(){
      ...
      try{const c=await API.createCampaign({name,masterPassword:this.state.createPassword,mode,templateId:mode==='template'?t?.id||'':'',heroIds:[...new Set(this.state.createHeroIds)],npcIds:[...new Set(this.state.createNpcIds||[])],villainIds:[...new Set(this.state.createVillainIds)]});rememberCampaign(c);this.setState(prev=>({campaigns:[c,...prev.campaigns.filter(x=>x.code!==c.code)],selectedCampaign:c,createOpen:false,createRosterKind:'',createRosterSearch:'',error:''}));this.toast(`Mesa criada: ${c.code}`);}catch(e){this.setState({error:e.message});}
    }
    async saveCampaignContent(content){
      if(this.state.profile?.role!=='master')return;
      const next={...(this.state.data.campaignContent||{}),...content,title:String(content?.title||'').trim()||this.state.selectedCampaign?.name||'Campanha',subtitle:String(content?.subtitle||'').trim(),summary:String(content?.summary||'').trim(),editorText:String(content?.editorText||''),rank:clamp(content?.rank||4,1,6),sessions:(content?.sessions||[]).map((row,i)=>({id:String(i+1).padStart(2,'0'),title:String(row?.title||`Sessão ${i+1}`).trim()||`Sessão ${i+1}`,text:String(row?.text||'')})),players:(this.state.data.heroes||[]).length,documentMode:content?.campaignPdf?'pdf':'editor'};
      this.setState({campaignSaving:true});
      try{
        await Promise.all([API.saveState('campaignContent',next),API.updateCampaignMeta({name:next.title})]);
        const selectedCampaign={...(this.state.selectedCampaign||{}),name:next.title,campaignContent:{...(this.state.selectedCampaign?.campaignContent||{}),title:next.title,subtitle:next.subtitle,rank:next.rank,players:next.players,mode:next.mode||'group'}};
        this.setState(prev=>({campaignSaving:false,campaignEditOpen:false,selectedCampaign,data:{...prev.data,campaignContent:next}}));this.toast('Campanha atualizada.');
      }catch(e){this.setState({campaignSaving:false});this.toast(e.message||'Não foi possível salvar a campanha.');}
    }
    ...
    async addRosterCharacter(kind,id,category=''){
      const library=this.creationLibrary(kind==='npc'?'hero':kind),source=library.find(item=>item.id===id);if(!source)return;
      const key=kind==='hero'?'heroes':kind==='npc'?'npcs':'villains';
      const sourceIsMinion=isMinionCharacter(source);
      const isRepeatableMinion=kind==='villain'&&(category==='minion'||sourceIsMinion);
      const allCampaign=[...(this.state.data.heroes||[]),...(this.state.data.npcs||[]),...(this.state.data.villains||[])];
      const sameSource=allCampaign.filter(item=>characterSourceId(item)===String(source.id));
      const heroMatch=(this.state.data.heroes||[]).find(item=>characterSourceId(item)===String(source.id));
      const npcMatch=(this.state.data.npcs||[]).find(item=>characterSourceId(item)===String(source.id));
      const villainMatch=(this.state.data.villains||[]).find(item=>characterSourceId(item)===String(source.id));
      if(!sourceIsMinion){
        if(kind==='npc'&&(heroMatch||villainMatch)){this.toast(`${source.n} já está na campanha como ${heroMatch?'Herói':'Vilão'}. Remova desse papel antes de adicioná-lo como NPC.`);return;}
        if((kind==='hero'||kind==='villain')&&npcMatch){this.toast(`${source.n} já está na campanha como NPC. Remova dos NPCs antes de mudar o papel.`);return;}
      }
      if(!isRepeatableMinion&&(this.state.data[key]||[]).some(item=>characterSourceId(item)===String(id)))return;
      ...
      if(isRepeatableMinion){
        ...
        entity.id=source.id;entity.sourceCharacterId=source.id;entity.campaignRole='npc';entity.rosterKind='npc';
      }else{
      ...
      try{
        let removal=null;
        const removalKeys=sourceIsMinion?[]:(kind==='hero'?['villains']:kind==='villain'?['heroes']:[]);
        for(const otherKey of removalKeys){const match=(this.state.data[otherKey]||[]).find(item=>characterSourceId(item)===String(id));if(!match)continue;removal=otherKey==='heroes'?await API.deleteHero(match.id):await API.deleteVillain(match.id);}
        if(kind==='hero')await API.saveHero(entity);else if(kind==='npc')await API.saveNpc(entity);else await API.saveVillain(entity);
        this.setState(prev=>{const data={...prev.data,scenario:normalizedScenario(removal?.scenario||prev.data.scenario),initiative:removal?.initiative||prev.data.initiative,combat:removal?.combat||prev.data.combat};for(const otherKey of removalKeys)data[otherKey]=(prev.data[otherKey]||[]).filter(x=>characterSourceId(x)!==String(id));data[key]=[...(prev.data[key]||[]),entity];if(kind==='hero')data.playerNotes={...(removal?.playerNotes||prev.data.playerNotes||{}),[entity.id]:''};return{rosterFocusCharacterId:source.id,rosterCategory:category||prev.rosterCategory||'all',data};});
        this.toast(isRepeatableMinion?`${entity.n} adicionado. Você pode adicionar quantos precisar.`:`${entity.n} adicionado como ${kind==='hero'?'herói':kind==='npc'?'NPC':'vilão'}.`);
      }catch(e){this.toast(e.message||'Não foi possível adicionar o personagem.');}
    }
    openNewCharacter(kind,category=''){
      const entity=blankCharacter(kind);
      if(kind==='npc'){entity.campaignRole='npc';entity.rosterKind='npc';}
      if(category==='minion'&&kind==='villain'){entity.tier='CAPANGA';entity.generic=true;entity.tags=['Villainous','Capanga'];}
      this.setState({rosterPicker:'',rosterSearch:'',rosterGroup:'',characterEditor:{kind,entity,isNew:true}});
    }
    ...
    async saveCharacterEditor(entity){
      const modal=this.state.characterEditor;if(!modal)return;const kind=modal.kind,key=kind==='hero'?'heroes':kind==='npc'?'npcs':'villains';
      if(kind==='npc'){entity={...entity,campaignRole:'npc',sourceCharacterId:entity.sourceCharacterId||entity.baseId||entity.id,rosterKind:'npc'};}
      try{if(kind==='hero')await API.saveHero(entity);else if(kind==='npc')await API.saveNpc(entity);else await API.saveVillain(entity);this.setState(prev=>{const scenario=normalizedScenario(prev.data.scenario),pieces=scenario.pieces.map(piece=>String(piece.characterId||piece.baseId||'')===String(entity.id)?{...piece,kind:kind==='npc'?'npc':piece.kind,name:entity.n||piece.name,image:entity.image||'',movement:{...(entity.movement||{})},maxHealth:entity.maxHealth??piece.maxHealth??null,currentHealth:entity.currentHealth??entity.maxHealth??piece.currentHealth??null,maxFocus:entity.maxFocus??piece.maxFocus??null,currentFocus:entity.currentFocus??entity.maxFocus??piece.currentFocus??null}:piece);return{characterEditor:null,data:{...prev.data,[key]:(prev.data[key]||[]).some(x=>x.id===entity.id)?(prev.data[key]||[]).map(x=>x.id===entity.id?entity:x):[...(prev.data[key]||[]),entity],scenario:{...scenario,pieces},...(kind==='hero'&&modal.isNew?{playerNotes:{...(prev.data.playerNotes||{}),[entity.id]:''}}:{})}};});this.toast('Ficha salva.');}catch(e){this.toast(e.message||'Não foi possível salvar a ficha.');}
    }
    async removeRosterCharacter(kind,entity){
      if(!entity||!confirm(`Remover ${entity.n} desta campanha?`))return;const key=kind==='hero'?'heroes':kind==='npc'?'npcs':'villains';
      try{const result=kind==='hero'?await API.deleteHero(entity.id):kind==='npc'?await API.deleteNpc(entity.id):await API.deleteVillain(entity.id);this.setState(prev=>({selectedPiece:null,data:{...prev.data,[key]:(prev.data[key]||[]).filter(x=>x.id!==entity.id),scenario:normalizedScenario(result?.scenario||prev.data.scenario),initiative:result?.initiative||prev.data.initiative,combat:result?.combat||prev.data.combat,...(kind==='hero'?{playerNotes:result?.playerNotes||prev.data.playerNotes}:{})}}),()=>this.ensureActor());this.toast(`${entity.n} removido.`);}catch(e){this.toast(e.message||'Não foi possível remover o personagem.');}
    }
    ...
    async adjustResource(deltaOrValue,resource,absolute=false){
      ...
      try{
        if(kind==='other'){
          ...
          const values={};values[curKey]=next;const updated=await API.adjustResources(kind,entity.id,values); const key=kind==='hero'?'heroes':kind==='npc'?'npcs':'villains';this.setState(prev=>({data:{...prev.data,[key]:(prev.data[key]||[]).map(x=>x.id===updated.id?updated:x)}}));
        }
    ...
    async runAction(ability,isCombat){
      ...
      try{
        ...
        else Object.assign(payload,{actorId:entity.id,kind:kind==='npc'?'npc':kind==='hero'?'hero':'villain',source:kind==='hero'?'hero':'threat',rollType:isCombat?'attack':'test',deferFinalize:true,actor:kind==='other'?{id:entity.id,n:entity.n,abilities:entity.abilities}:undefined});
    ...
    async movePiece(x,y,mode='run',pieceId=''){
      if(this.state.scenarioMoveBusy)return;
      const previous=normalizedScenario(this.state.data.scenario),owned=this.state.profile.role==='player'?previous.pieces.find(p=>p.kind==='hero'&&String(p.characterId||p.baseId)===String(this.state.profile.heroId||'')):null,id=pieceId||this.state.selectedPiece||owned?.id;if(!id)return;
      const piece=previous.pieces.find(p=>p.id===id);if(!piece)return;
      // Movimento otimista: a peça responde no clique, enquanto o servidor valida em segundo plano.
      const optimistic=normalizedScenario(previous);
      optimistic.pieces=optimistic.pieces.map(p=>p.id===id?{...p,x:Number(x),y:Number(y)}:p);
      this.setState(prev=>({scenarioMoveBusy:true,data:{...prev.data,scenario:optimistic},selectedPiece:id,moveMode:mode}));
      try{
        const result=await API.moveScenarioPiece({pieceId:id,x,y,mode,from:{x:piece.x,y:piece.y}});
        this.setState(prev=>({scenarioMoveBusy:false,data:{...prev.data,scenario:normalizedScenario(result.scenario||prev.data.scenario)},selectedPiece:id,moveMode:mode}));
      }catch(e){
        this.setState(prev=>({scenarioMoveBusy:false,data:{...prev.data,scenario:previous}}));
    ...
    async saveScenario(next,message=''){
      if(this.state.profile.role!=='master')return;
      clearTimeout(this.scenarioSaveTimer);this.scenarioSavePending=null;
    ...
    queueScenarioSave(next){
      if(this.state.profile.role!=='master')return;
      // Pintura do mapa não recalcula/reposiciona todas as peças a cada clique.
      // Reposicionamento seguro fica restrito a resize/modelos/geração e ações estruturais.
      const scenario=normalizedScenario(next);
      this.scenarioSavePending=scenario;
      this.setState(prev=>({data:{...prev.data,scenario}}));
      clearTimeout(this.scenarioSaveTimer);
      this.scenarioSaveTimer=setTimeout(async()=>{
        const pending=this.scenarioSavePending;if(!pending)return;
        try{await API.saveState('scenario',pending);if(this.scenarioSavePending===pending)this.scenarioSavePending=null;}
        catch(e){this.scenarioSavePending=null;this.toast(e.message||'Não foi possível salvar o cenário.');}
      },180);
    }
    ...
    async editScenarioCell(x,y,tool,piece){
      ...
      else if(OBSTACLE_META[tool]){if(piece&&OBSTACLE_META[tool].block){this.toast('Mova a peça antes de colocar um obstáculo nessa casa.');return;}delete next.decor[key];next.obstacles[key]=tool;}
    ...
    async addScenarioPieces(){
      const current=normalizedScenario(this.state.data.scenario),fallback=(this.state.data.heroes||[])[0]?`hero|${this.state.data.heroes[0].id}`:(this.state.data.npcs||[])[0]?`npc|${this.state.data.npcs[0].id}`:(this.state.data.villains||[])[0]?`villain|${this.state.data.villains[0].id}`:'',choice=this.state.scenarioPieceChoice||fallback;if(!choice)return this.toast('Adicione um personagem à campanha primeiro.');const[kind,id]=choice.split('|');let entity=null;if(kind==='hero')entity=(this.state.data.heroes||[]).find(e=>e.id===id);else if(kind==='npc')entity=(this.state.data.npcs||[]).find(e=>e.id===id);else if(kind==='villain')entity=(this.state.data.villains||[]).find(e=>e.id===id);if(!entity)return this.toast('Escolha um personagem.');
      ...
      for(let i=0;i<qty&&spots.length;i++){
        while(cursor<spots.length&&occupied.has(scenarioKey(spots[cursor].x,spots[cursor].y)))cursor++;
        const spot=spots[cursor]||spots.find(c=>!occupied.has(scenarioKey(c.x,c.y)));if(!spot)break;cursor++;occupied.add(scenarioKey(spot.x,spot.y));const pieceKind=kind==='hero'?'hero':kind==='npc'?'npc':'enemy';
        next.pieces.push({id:`piece-${entity.id}-${Date.now()}-${i}-${Math.random().toString(36).slice(2,6)}`,kind:pieceKind,tier:(kind==='other'||entity.generic===true||String(entity.tier||'').toUpperCase()==='CAPANGA')?'minion':kind==='villain'?'main':undefined,baseId:entity.id,characterId:entity.id,name:qty>1?`${entity.n} ${i+1}`:entity.n,short:monogram(entity.n),image:entity.image||'',x:spot.x,y:spot.y,movement:{...(entity.movement||{})},maxHealth:entity.maxHealth??null,currentHealth:entity.currentHealth??entity.maxHealth??null,maxFocus:entity.maxFocus??null,currentFocus:entity.currentFocus??entity.maxFocus??null});added++;
      }
    ...
    renderLogin(){
      ...
      const catalog=this.creationLibrary('hero');
      const rosterRole=s.createRosterKind||'',selectedIds=rosterRole==='hero'?s.createHeroIds:rosterRole==='npc'?(s.createNpcIds||[]):s.createVillainIds;
      const rosterTerm=(s.createRosterSearch||'').trim().toLocaleLowerCase('pt-BR');
      const creationSelectedIds=allSelectedCharacterIds(s);
      const rosterGroups=rosterRole&&INTEL?.collectGroups?INTEL.collectGroups(catalog,{}):[];
      const rankedCreation=INTEL?.rankItems?INTEL.rankItems(catalog,{focusId:s.createFocusCharacterId,selectedIds:creationSelectedIds,templates:s.templates}):catalog.map(item=>({item,score:0,reasons:[]}));
      const rosterFiltered=rankedCreation.filter(row=>rosterRole==='minion'?catalogCategory(row.item)==='minion':rosterRole==='villain'?catalogCategory(row.item)!=='minion':true).filter(row=>!s.createRosterGroup||(INTEL?.matchesGroup?INTEL.matchesGroup(row.item,s.createRosterGroup):true)).filter(row=>!rosterTerm||`${row.item.n||''} ${row.item.r||''} ${row.item.role||''} ${row.item.teams||''}`.toLocaleLowerCase('pt-BR').includes(rosterTerm));
      const selectedNames=role=>{const ids=role==='hero'?s.createHeroIds:role==='npc'?(s.createNpcIds||[]):s.createVillainIds;return catalog.filter(item=>ids.includes(item.id)&&(role!=='minion'||catalogCategory(item)==='minion')&&(role!=='villain'||catalogCategory(item)!=='minion'));};
      const focusCharacter=catalog.find(item=>item.id===s.createFocusCharacterId)||null;
      return h('div',{className:'min-h-screen bg-[radial-gradient(circle_at_70%_20%,rgba(239,51,64,.12),transparent_35%),#080b10] p-4 text-white'},h('div',{className:'mx-auto max-w-6xl py-8 sm:py-14'},[
        h('div',{key:'head',className:'mb-8'},[h(TinyLabel,{key:'s',className:'text-[#ff6a75]'},'MARVEL MULTIVERSE RPG'),h('h1',{key:'t',className:'mt-2 text-4xl font-black tracking-tight sm:text-5xl'},'Projeto Arachne'),h('p',{key:'p',className:'mt-2 text-sm text-[#8b95a3]'},'Escolha uma campanha existente ou monte uma nova mesa com os personagens que você quer usar.')]),
        ...
        h('div',{key:'campaigns',className:'grid gap-3 md:grid-cols-2'},s.campaigns.length?s.campaigns.map(c=>h('button',{key:c.code,onClick:()=>this.openCampaign(c.code),className:'flex items-center gap-4 rounded-2xl border border-white/10 bg-[#10151d] p-4 text-left transition hover:-translate-y-0.5 hover:border-[#ef3340]/50'},[h('span',{key:'a',className:'grid h-12 w-12 place-items-center rounded-xl border border-[#ef3340]/50 bg-[#ef3340]/10 font-black'},c.code.slice(0,2)),h('div',{key:'t',className:'min-w-0 flex-1'},[h('b',{key:'n',className:'block truncate'},c.name),h('small',{key:'c',className:'text-[10px] text-[#788291]'},`CÓDIGO ${c.code}`)]),h('span',{key:'x'},'→')])):h(Card,{className:'p-6 text-sm text-[#798392]'},'Nenhuma campanha encontrada. Crie sua primeira mesa abaixo.')),
        h('div',{key:'actions',className:'mt-5 flex flex-wrap gap-2'},[h(Button,{key:'join',onClick:()=>this.setState({accessMode:s.accessMode==='join'?'':'join'})},'ENTRAR COM CÓDIGO'),h(Button,{key:'create',primary:true,onClick:()=>this.setState(prev=>({createOpen:!prev.createOpen,createName:prev.createName||'Nova Campanha',createMode:prev.createMode||'blank'}))},s.createOpen?'FECHAR CRIAÇÃO':'CRIAR CAMPANHA')]),
        ...
        s.createOpen?h(Card,{key:'createbox',className:'mt-5 overflow-hidden'},[
          h('div',{key:'ch',className:'border-b border-white/10 p-4 sm:p-5'},[h(TinyLabel,{key:'l'},'NOVA CAMPANHA'),h('h2',{key:'h',className:'mt-1 text-xl font-black'},'Monte sua mesa'),h('p',{key:'p',className:'mt-1 text-xs leading-5 text-[#7e8997]'},'Comece vazia ou use um modelo. Depois escolha Heróis, NPCs, Vilões e Capangas para a campanha.')]),
          h('div',{key:'body',className:'space-y-5 p-4 sm:p-5'},[
            h('div',{key:'base'},[h(TinyLabel,{key:'l'},'1 · BASE DA CAMPANHA'),h('div',{key:'g',className:'mt-2 grid gap-2 md:grid-cols-2 xl:grid-cols-3'},[
              h('button',{key:'blank',type:'button',onClick:()=>this.chooseCreationBase(''),className:cx('rounded-xl border p-3 text-left',s.createMode==='blank'?'border-[#ef3340] bg-[#ef3340]/10':'border-white/10 bg-[#10161e]')},[h('small',{key:'m',className:'text-[9px] font-black text-[#7e8795]'},'DO ZERO'),h('b',{key:'n',className:'mt-1 block text-sm'},'Campanha vazia'),h('span',{key:'s',className:'mt-1 block text-[10px] leading-4 text-[#7b8593]'},'Você escolhe os personagens e configura o restante depois.')]),
              ...s.templates.map(t=>h('button',{key:t.id,type:'button',onClick:()=>this.chooseCreationBase(t.id),className:cx('rounded-xl border p-3 text-left',s.createMode==='template'&&s.createTemplate===t.id?'border-[#ef3340] bg-[#ef3340]/10':'border-white/10 bg-[#10161e]')},[h('small',{key:'m',className:'text-[9px] font-black text-[#7e8795]'},`${t.mode==='solo'?'SOLO':`${t.players||0} JOGADORES`} · RANK ${t.rank||4}`),h('b',{key:'n',className:'mt-1 block text-sm'},t.name),h('span',{key:'s',className:'mt-1 block text-[10px] leading-4 text-[#7b8593]'},t.subtitle||t.summary||'Modelo de campanha')]))
            ])]),
            h('div',{key:'details',className:'grid gap-3 sm:grid-cols-2'},[h('label',{key:'n',className:'grid gap-1'},[h(TinyLabel,{key:'l'},'2 · NOME'),h('input',{key:'i',value:s.createName,onChange:e=>this.setState({createName:e.target.value}),placeholder:'Nome da campanha',className:'min-h-11 rounded-xl border border-white/10 bg-black/20 px-3 outline-none focus:border-[#ef3340]/60'})]),h('label',{key:'p',className:'grid gap-1'},[h(TinyLabel,{key:'l'},'SENHA DO MESTRE'),h('input',{key:'i',type:'password',value:s.createPassword,onChange:e=>this.setState({createPassword:e.target.value}),placeholder:'Senha do Mestre',className:'min-h-11 rounded-xl border border-white/10 bg-black/20 px-3 outline-none focus:border-[#ef3340]/60'})])]),
            h('div',{key:'roster'},[h(TinyLabel,{key:'l'},'3 · PERSONAGENS POR CATEGORIA'),h('p',{key:'intro',className:'mt-1 text-xs text-[#778291]'},'Defina quem será Herói, NPC, Vilão ou Capanga. Personagens comuns não podem ser NPC e Herói/Vilão ao mesmo tempo; capangas podem coexistir em mais de um papel.'),h('div',{key:'cards',className:'mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4'},[
              ['hero','Heróis','Personagens jogáveis do lado dos jogadores.'],['npc','NPCs','Aliados, neutros e personagens narrativos controlados pelo Mestre.'],['villain','Vilões','Rivais, ameaças e chefes controlados pelo Mestre.'],['minion','Capangas','Unidades repetíveis, agentes, soldados, drones e criaturas.']
            ].map(([cat,label,desc])=>{const chosen=selectedNames(cat);return h('button',{key:cat,type:'button',onClick:()=>this.setState({createRosterKind:s.createRosterKind===cat?'':cat,createRosterSide:'',createRosterSearch:'',createRosterGroup:''}),className:cx('rounded-xl border p-4 text-left transition',s.createRosterKind===cat?'border-[#ef3340] bg-[#ef3340]/8':'border-white/10 bg-[#0e141c] hover:border-white/20')},[h('div',{key:'h',className:'flex items-center justify-between gap-2'},[h('b',{key:'n'},label),h('span',{key:'c',className:cx('rounded-full border px-2 py-1 text-[10px] font-black',categoryTone(cat))},chosen.length)]),h('p',{key:'p',className:'mt-1 text-[11px] leading-4 text-[#788392]'},desc),h('div',{key:'sel',className:'mt-3 flex flex-wrap gap-1'},chosen.slice(0,4).map(x=>h('span',{key:x.id,className:'rounded-md bg-white/[.05] px-2 py-1 text-[9px]'},x.n)).concat(chosen.length>4?[h('span',{key:'more',className:'rounded-md bg-white/[.05] px-2 py-1 text-[9px]'},`+${chosen.length-4}`)]:[]))]);})),
            rosterRole?h('div',{key:'picker',className:'mt-3 rounded-xl border border-white/10 bg-black/15 p-3'},[
              h('div',{key:'head',className:'flex flex-wrap items-center justify-between gap-2'},[h('div',{key:'t'},[h(TinyLabel,{key:'l'},`ESCOLHER ${catalogCategoryLabel(rosterRole).toUpperCase()}`),h('span',{key:'n',className:'mt-1 block text-xs text-[#7d8795]'},`${selectedNames(rosterRole).length} selecionado(s) para ${catalogCategoryLabel(rosterRole).toLocaleLowerCase('pt-BR')}`)]),h('div',{key:'tools',className:'flex w-full flex-wrap gap-2'},[h('input',{key:'search',value:s.createRosterSearch,onChange:e=>this.setState({createRosterSearch:e.target.value}),placeholder:'Buscar personagem ou equipe...',className:'min-h-10 min-w-[220px] flex-1 rounded-lg border border-white/10 bg-[#0c1118] px-3 text-sm outline-none sm:w-72'}),h('select',{key:'group',value:s.createRosterGroup||'',onChange:e=>this.setState({createRosterGroup:e.target.value}),className:'min-h-10 rounded-lg border border-white/10 bg-[#111720] px-3 text-xs font-bold'},[h('option',{key:'all',value:''},'TODOS OS GRUPOS'),...rosterGroups.map(g=>h('option',{key:g.id,value:g.id},`${g.label} (${g.count})`))])])]),
              focusCharacter?h('div',{key:'focus',className:'mt-3 flex items-center gap-3 rounded-xl border border-[#ef3340]/25 bg-[#ef3340]/5 p-3'},[h(Portrait,{key:'p',entity:focusCharacter,size:'sm'}),h('div',{key:'t',className:'min-w-0'},[h(TinyLabel,{key:'l',className:'text-[#ff7a84]'},'FOCO INTELIGENTE'),h('b',{key:'n',className:'mt-1 block truncate text-xs'},focusCharacter.n),h('span',{key:'s',className:'mt-1 block text-[9px] text-[#8a95a4]'},'Parceiros, grupos, rivais e capangas ligados a ele recebem prioridade nesta lista.')])]):null,
              h('div',{key:'grid',className:'campaign-create-roster-scroll character-scroll-area mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3'},rosterFiltered.length?rosterFiltered.map(row=>{const item=row.item,active=selectedIds.includes(item.id),sourceIsMinion=isMinionCharacter(item),blocked=!sourceIsMinion&&!active&&(rosterRole==='npc'?((s.createHeroIds||[]).includes(item.id)||(s.createVillainIds||[]).includes(item.id)):((rosterRole==='hero'||rosterRole==='villain')&&(s.createNpcIds||[]).includes(item.id))),blockedLabel=blocked?(rosterRole==='npc'?'JÁ É HERÓI/VILÃO':'JÁ É NPC'):'',reasons=row.reasons||[];return h('button',{key:item.id,type:'button',disabled:blocked,onClick:()=>!blocked&&this.toggleCreationCharacter(rosterRole,item.id),className:cx('relative flex items-center gap-3 rounded-xl border p-2 text-left transition',blocked?'cursor-not-allowed border-white/5 bg-white/[.02] opacity-45':active?'border-emerald-700/60 bg-emerald-950/20':row.score>0?'border-[#ef3340]/25 bg-[#171d27] hover:border-[#ef3340]/55':'border-white/10 bg-[#111720] hover:border-white/20')},[row.score>0&&!active&&!blocked?h('span',{key:'rel',className:'shrink-0 rounded-md border border-[#ef3340]/25 bg-[#ef3340]/10 px-1.5 py-0.5 text-[7px] font-black text-[#ff7b85]'},'RELACIONADO'):null,h(Portrait,{key:'p',entity:item,size:'sm'}),h('div',{key:'t',className:'min-w-0 flex-1'},[h('b',{key:'n',className:'block truncate text-xs'},item.n),h('small',{key:'r',className:'block truncate text-[9px] text-[#75808f]'},item.r||`Rank ${item.rank||4}`),blocked?h('span',{key:'blocked',className:'mt-1 block truncate text-[8px] font-black text-amber-300'},blockedLabel):reasons.length?h('span',{key:'why',className:'mt-1 block truncate text-[8px] text-[#e7959b]'},reasons.join(' · ')):item.teams?h('span',{key:'team',className:'mt-1 block truncate text-[8px] text-[#687281]'},item.teams):null]),h('span',{key:'c',className:cx('grid h-6 w-6 shrink-0 place-items-center rounded-full border text-[10px] font-black',blocked?'border-amber-700/50 text-amber-300':active?'border-emerald-500 text-emerald-400':'border-white/15 text-[#667180]')},blocked?'!':active?'✓':'+')]);}):h('div',{className:'col-span-full py-8 text-center text-xs text-[#74808f]'},'Nenhum personagem encontrado nesta categoria / grupo.'))
            ]):null
            ]),
            h('div',{key:'finish',className:'flex flex-col gap-2 border-t border-white/10 pt-4 sm:flex-row sm:items-center sm:justify-between'},[h('p',{key:'p',className:'text-xs text-[#7d8795]'},`${selectedNames('hero').length} heróis · ${selectedNames('npc').length} NPCs · ${selectedNames('villain').length} vilões · ${selectedNames('minion').length} capangas`),h(Button,{key:'b',primary:true,onClick:()=>this.createCampaign(),className:'sm:min-w-52'},'CRIAR MESA')])
          ])
        ]):null
      ]));
    }
    ...
    renderCentral(){
      ...
      if(isMaster){
        identityPanel=h(Card,{key:'actors',className:'p-4 sm:p-5'},[
          h('div',{key:'actorHead',className:'flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'},[
            h('div',{key:'label'},[h(TinyLabel,{key:'l'},'QUEM ESTÁ AGINDO?'),h('span',{key:'hint',className:'mt-1 block text-[10px] text-[#687382]'},'Escolha o grupo controlado pelo Mestre.')]),
            h('div',{key:'switch',className:'grid w-full grid-cols-2 rounded-xl border border-white/10 bg-black/25 p-1 sm:w-auto sm:min-w-[280px]'},[
              h('button',{key:'villain',type:'button','aria-pressed':s.masterActorView==='villain',onClick:()=>this.setState({masterActorView:'villain',actorKey:'',roll:null,power:''},()=>this.ensureActor()),className:cx('min-h-10 rounded-lg px-5 py-2 text-xs font-black tracking-wide transition',s.masterActorView==='villain'?'bg-[#ef3340] text-white shadow-[0_8px_24px_rgba(239,51,64,.18)]':'text-[#85909f] hover:bg-white/[.04]')},'VILÕES'),
              h('button',{key:'npc',type:'button','aria-pressed':s.masterActorView==='npc',onClick:()=>this.setState({masterActorView:'npc',actorKey:'',roll:null,power:''},()=>this.ensureActor()),className:cx('min-h-10 rounded-lg px-5 py-2 text-xs font-black tracking-wide transition',s.masterActorView==='npc'?'bg-[#ef3340] text-white shadow-[0_8px_24px_rgba(239,51,64,.18)]':'text-[#85909f] hover:bg-white/[.04]')},'NPCS')
            ])
          ]),
          h('div',{key:'x',className:'mt-3'},this.masterActors().length?h(CharacterStrip,{items:this.masterActors(),selectedKey:s.actorKey,onSelect:key=>this.setState({actorKey:key,roll:null,power:''})}):h('div',{className:'rounded-xl border border-dashed border-white/10 p-5 text-center text-xs text-[#74808f]'},s.masterActorView==='npc'?'Nenhum NPC nesta campanha.':'Nenhum vilão nesta campanha.')),
          entity?h('div',{key:'selected',className:'mt-4 grid gap-4 border-t border-white/5 pt-4 lg:grid-cols-[auto_1fr] lg:items-center'},[
            h('div',{key:'id',className:'flex items-center gap-4'},[h(Portrait,{key:'p',entity,size:'xl'}),h('div',{key:'t'},[h(TinyLabel,{key:'l'},kind==='other'?'CAPANGA':kind==='npc'?'NPC':kind==='hero'?'HERÓI':'VILÃO'),h('b',{key:'n',className:'mt-1 block text-xl sm:text-2xl'},entity.n),h('span',{key:'r',className:'text-sm text-[#7e8896]'},entity.r||entity.role||'')])]),
      ...
      return h('div',{className:'space-y-4'},[
        ...
        isMaster?h(InitiativePanel,{key:'init',initiative:d.initiative||[],choices:[...(d.heroes||[]),...(d.npcs||[]),...(d.villains||[])],onAdd:id=>this.initAdd(id),onRemove:id=>this.initRemove(id),onRoll:id=>this.initRoll(id),onRollAll:()=>this.initRollAll(),lastRoll:s.initiativeRoll,animating:s.initiativeAnimating}):null,
        identityPanel,
        ...
        h('div',{key:'board'},h(MemoBoard,{scenario:d.scenario,heroes:d.heroes,npcs:d.npcs||[],villains:d.villains,role:s.profile.role,heroId:s.profile.heroId,selected:s.selectedPiece,onSelect:id=>this.setState({selectedPiece:id}),onMove:(x,y,mode,pieceId)=>this.movePiece(x,y,mode,pieceId),zoom:s.zoom,onZoom:zoom=>this.setState({zoom}),onReset:()=>this.resetMovement('',isMaster),onResetPiece:id=>this.resetMovement(id,false),moveMode:s.moveMode,onMoveMode:moveMode=>this.setState({moveMode}),moving:s.scenarioMoveBusy})),
        isMaster?h(RollHistory,{key:'hist',rows:d.actionHistory||[]}):null
      ]);
    }

    renderRoster(kind){
      const isHero=kind==='hero',isNpc=kind==='npc';
      const items=isHero?this.state.data.heroes:isNpc?(this.state.data.npcs||[]):this.state.data.villains,isMaster=this.state.profile?.role==='master';
      const title=isHero?'Heróis':isNpc?'NPCs':'Vilões',section=isHero?'DOSSIÊS':isNpc?'CONTROLADOS PELO MESTRE':'AMEAÇAS',singular=isHero?'HERÓI':isNpc?'NPC':'VILÃO';
      return h('div',{className:'space-y-5'},[
        h('div',{key:'head',className:'flex flex-wrap items-end justify-between gap-3'},[
          h('div',{key:'t'},[h(TinyLabel,{key:'s'},section),h('h1',{key:'h',className:'mt-1 text-3xl font-black'},title),h('p',{key:'p',className:'mt-1 text-sm text-[#7d8795]'},isNpc?'Personagens aliados, neutros ou narrativos controlados pelo Mestre.':isMaster?'Gerencie os personagens que pertencem a esta campanha.':'Personagens disponíveis nesta campanha.')]),
          isMaster?h(Button,{key:'add',primary:true,onClick:()=>this.setState({rosterPicker:kind,rosterCategory:isHero?'hero':isNpc?'all':'villain',rosterSearch:'',rosterGroup:''})},`+ ADICIONAR ${singular}`):null
        ]),
        h('div',{key:'g',className:'character-management-grid character-scroll-area grid gap-3 sm:grid-cols-2 xl:grid-cols-3'},items.length?items.map(entity=>h(Card,{key:entity.id,className:'overflow-hidden'},[
          h('button',{key:'open',onClick:()=>this.openSheet(entity,kind),className:'flex w-full items-center gap-4 p-4 text-left'},[h(Portrait,{key:'p',entity,size:'md'}),h('div',{key:'t',className:'min-w-0 flex-1'},[h(TinyLabel,{key:'l',className:isNpc?'text-sky-300':''},isHero?'HERÓI':isNpc?'NPC':entity.tier||'VILÃO'),h('b',{key:'n',className:'mt-1 block truncate text-base'},entity.n),h('span',{key:'r',className:'block truncate text-xs text-[#7c8694]'},entity.r||entity.role||''),h('small',{key:'v',className:'mt-2 block text-[10px] text-[#8c95a2]'},`HP ${entity.currentHealth??entity.maxHealth}/${entity.maxHealth} · FO ${entity.currentFocus??entity.maxFocus}/${entity.maxFocus}`)])]),
          isMaster?h('div',{key:'a',className:'flex gap-2 border-t border-white/5 p-2'},[h(Button,{key:'e',onClick:()=>this.openCharacterEditor(kind,entity),className:'min-h-9 flex-1 py-1'},'EDITAR'),h(Button,{key:'r',danger:true,onClick:()=>this.removeRosterCharacter(kind,entity),className:'min-h-9 px-3 py-1'},'REMOVER')]):null
        ])):h(Card,{className:'p-8 text-center text-sm text-[#74808f]'},isMaster?`Nenhum ${isHero?'herói':isNpc?'NPC':'vilão'} nesta campanha. Use o botão Adicionar.`:'Nenhum personagem disponível.'))
      ]);
    }
    renderCampaign(){
      const s=this.state,d=s.data,c=d.campaignContent||{},sessions=c.sessions||[],isMaster=s.profile?.role==='master';
      if(isMaster&&s.campaignEditOpen)return h(CampaignEditor,{content:c,campaign:s.selectedCampaign,heroes:d.heroes||[],npcs:d.npcs||[],villains:d.villains||[],saving:s.campaignSaving,onSave:content=>this.saveCampaignContent(content),onCancel:()=>this.setState({campaignEditOpen:false}),onAddCategory:category=>this.setState({rosterPicker:category==='minion'?'villain':category,rosterCategory:category==='minion'?'minion':'all',rosterSearch:'',rosterGroup:'',rosterFocusCharacterId:this.state.rosterFocusCharacterId||''}),onEditCharacter:(kind,entity)=>this.openCharacterEditor(kind,entity),onRemoveCharacter:(kind,entity)=>this.removeRosterCharacter(kind,entity),onUploadPdf:file=>this.uploadCampaignPdf(file)});
      const heroRows=(d.heroes||[]).map(entity=>({entity,side:'hero'})),npcRows=(d.npcs||[]).map(entity=>({entity,side:'npc'})),villainRows=(d.villains||[]).map(entity=>({entity,side:'villain'}));
      const rosterBlock=category=>{let items=[];if(category==='hero')items=heroRows;else if(category==='npc')items=npcRows;else if(category==='minion')items=villainRows.filter(row=>catalogCategory(row.entity)==='minion');else items=villainRows.filter(row=>catalogCategory(row.entity)!=='minion');const roleLabel=side=>side==='hero'?'JOGÁVEL':side==='npc'?'NPC · MESTRE':'AMEAÇA';return h(Card,{className:'p-4'},[h('div',{key:'h',className:'flex items-center justify-between gap-2'},[h('div',{key:'t'},[h(TinyLabel,{key:'l'},catalogCategoryLabel(category).toUpperCase()),h('b',{key:'n',className:'mt-1 block text-sm'},`${items.length} na campanha`)]),isMaster?h(Button,{key:'a',onClick:()=>this.setState({campaignEditOpen:true}),className:'min-h-8 px-2 py-1'},'GERENCIAR'):null]),h('div',{key:'g',className:'campaign-overview-roster-scroll character-scroll-area mt-3 grid gap-2 sm:grid-cols-2'},items.length?items.map(({entity,side})=>h('button',{key:`${side}-${entity.id}`,onClick:()=>this.openSheet(entity,side),className:'flex items-center gap-3 rounded-xl border border-white/10 bg-black/15 p-2 text-left hover:border-white/20'},[h(Portrait,{key:'p',entity,size:'sm'}),h('div',{key:'t',className:'min-w-0'},[h('b',{key:'n',className:'block truncate text-xs'},entity.n),h('small',{key:'r',className:'block truncate text-[9px] text-[#74808f]'},`${entity.r||`Rank ${entity.rank||4}`} · ${roleLabel(side)}`)])])):h('p',{className:'py-4 text-center text-xs text-[#74808f]'},'Nenhum personagem.'))]);};
      return h('div',{className:'space-y-5'},[
        ...
        h('div',{key:'m',className:'flex flex-wrap gap-2'},[c.rank&&h('span',{key:'r',className:'rounded-lg border border-white/10 px-3 py-2 text-xs'},`RANK ${c.rank}`),c.mode&&h('span',{key:'o',className:'rounded-lg border border-white/10 px-3 py-2 text-xs'},c.mode==='solo'?'SOLO':'GRUPO'),s.selectedCampaign?.code&&h('span',{key:'c',className:'rounded-lg border border-white/10 px-3 py-2 text-xs'},`CÓDIGO ${s.selectedCampaign.code}`),c.campaignPdf&&h('a',{key:'pdf',href:c.campaignPdf,target:'_blank',rel:'noopener',className:'rounded-lg border border-[#ef3340]/40 bg-[#ef3340]/10 px-3 py-2 text-xs font-black'},'ABRIR PDF')]),
        c.editorText?h(Card,{key:'txt',className:'p-4 sm:p-5'},[h(TinyLabel,{key:'l'},'NOTAS DA CAMPANHA'),h('p',{key:'p',className:'mt-2 whitespace-pre-wrap text-sm leading-6 text-[#a0a9b6]'},c.editorText)]):null,
        h('div',{key:'roster',className:'grid gap-4 xl:grid-cols-2'},[rosterBlock('hero'),rosterBlock('npc'),rosterBlock('villain'),rosterBlock('minion')]),
        h('div',{key:'sessions'},[h('div',{key:'h',className:'flex items-center justify-between'},[h('div',{key:'t'},[h(TinyLabel,{key:'l'},'SESSÕES'),h('b',{key:'b',className:'mt-1 block'},'Estrutura da campanha')])]),h('div',{key:'list',className:'mt-3 space-y-2'},sessions.length?sessions.map((session,i)=>h(Card,{key:session.id||i,className:'p-4'},[h(TinyLabel,{key:'i'},session.id||String(i+1).padStart(2,'0')),h('b',{key:'t',className:'mt-1 block'},session.title),h('p',{key:'p',className:'mt-2 whitespace-pre-wrap text-sm leading-6 text-[#858f9d]'},session.text)])):h(Card,{className:'p-6 text-center text-sm text-[#74808f]'},'Nenhuma sessão cadastrada.'))])
      ]);
    }
    renderScenarioPage(){
    ...
    renderPage(){switch(this.state.page){case'heroes':return this.renderRoster('hero');case'npcs':return this.renderRoster('npc');case'villains':return this.renderRoster('villain');case'campaign':return this.renderCampaign();case'scenario':return this.renderScenarioPage();case'rules':return this.renderRules();case'notes':return this.renderNotes();default:return this.renderCentral();}}
    renderApp(){const s=this.state,campaign=s.profile?.campaign||s.selectedCampaign;return h('div',{className:'min-h-screen bg-[#080b10] text-white'},[
      ...
      s.rosterPicker?h(CharacterPickerModal,{key:'picker',kind:s.rosterPicker,items:this.creationLibrary(s.rosterPicker==='npc'?'hero':s.rosterPicker),existing:new Set((s.rosterPicker==='hero'?s.data.heroes:s.rosterPicker==='npc'?(s.data.npcs||[]):s.data.villains).map(item=>item.id)),search:s.rosterSearch,onSearch:rosterSearch=>this.setState({rosterSearch}),category:s.rosterCategory||'all',onCategory:rosterCategory=>this.setState({rosterCategory,rosterGroup:''}),group:s.rosterGroup,onGroup:rosterGroup=>this.setState({rosterGroup}),selectedIds:[...(s.data.heroes||[]),...(s.data.npcs||[]),...(s.data.villains||[])].map(item=>item.id),campaignItems:[...(s.data.heroes||[]),...(s.data.npcs||[]),...(s.data.villains||[])],campaignRosters:{heroes:s.data.heroes||[],npcs:s.data.npcs||[],villains:s.data.villains||[]},focusId:s.rosterFocusCharacterId,templates:s.templates,onAdd:(id,category)=>this.addRosterCharacter(s.rosterPicker,id,category),onCreate:()=>this.openNewCharacter(s.rosterPicker,s.rosterCategory||'all'),onClose:()=>this.setState({rosterPicker:'',rosterSearch:'',rosterGroup:''})}):null,
