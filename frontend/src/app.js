(() => {
  ...
  function CharacterPickerModal({kind,items,existing,onAdd,onCreate,onClose,search='',onSearch,category='',onCategory,group='',onGroup,selectedIds=[],campaignItems=[],focusId='',templates=[]}){
    ...
    return h('div',{className:'character-picker-overlay fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-3',onMouseDown:e=>{if(e.target===e.currentTarget)onClose();}},h('div',{className:'character-picker-modal w-full max-w-6xl overflow-hidden rounded-2xl border border-white/10 bg-[#0d1118] shadow-2xl'},[
      h('div',{key:'h',className:'character-picker-header flex flex-wrap items-center justify-between gap-3 border-b border-white/10 p-4 sm:p-5'},[h('div',{key:'t'},[h(TinyLabel,{key:'l'},'CATÁLOGO INTELIGENTE'),h('h2',{key:'n',className:'mt-1 text-xl font-black'},title),h('p',{key:'p',className:'mt-1 text-xs text-[#7e8997]'},`A biblioteca é compartilhada: qualquer personagem pode entrar como herói ou vilão e também pode ser usado como NPC. Aqui ele será adicionado à campanha como ${kind==='hero'?'Herói jogável':kind==='npc'?'NPC controlado pelo Mestre':'Vilão/ameaça'}.`)]),h(Button,{key:'x',onClick:onClose,className:'px-3'},'×')]),
      ...
      h('div',{key:'g',className:'catalog-grid-scroll character-scroll-area character-picker-list grid gap-2 p-4 sm:grid-cols-2 lg:grid-cols-3'},filtered.length?filtered.map(row=>{const item=row.item,isRepeatableMinion=kind==='villain'&&activeCategory==='minion',instanceCount=isRepeatableMinion?(campaignItems||[]).filter(x=>String(x?.sourceCharacterId||x?.baseId||x?.id||'')===String(item.id||'')).length:0,used=!isRepeatableMinion&&existing.has(item.id),reasons=row.reasons||[],officialGroups=INTEL?.officialGroups?INTEL.officialGroups(item):String(item.teams||'').split(',').map(x=>x.trim()).filter(Boolean);return h('button',{key:item.id,type:'button',disabled:used,onClick:()=>onAdd(item.id,activeCategory),className:cx('relative flex items-center gap-3 rounded-xl border p-3 text-left transition',used?'cursor-not-allowed border-white/5 bg-white/[.02] opacity-45':row.score>0?'border-[#ef3340]/25 bg-[#171d27] hover:border-[#ef3340]/55':'border-white/10 bg-[#111720] hover:border-[#ef3340]/55')},[
  ...
  class ArachneApp extends React.Component {
    ...
    toggleCreationCharacter(kind,id){
      ...
      this.setState(prev=>{const removing=(prev[target]||[]).includes(id),next={createHeroIds:[...(prev.createHeroIds||[])],createNpcIds:[...(prev.createNpcIds||[])],createVillainIds:[...(prev.createVillainIds||[])]};if(removing)next[target]=next[target].filter(x=>x!==id);else{for(const key of ['createHeroIds','createNpcIds','createVillainIds'])next[key]=next[key].filter(x=>x!==id);next[target].push(id);}return{...next,createFocusCharacterId:removing&&prev.createFocusCharacterId===id?'':id};});
    }
    ...
    async addRosterCharacter(kind,id,category=''){
      ...
      const otherKeys=['heroes','npcs','villains'].filter(k=>k!==key);
      const isRepeatableMinion=kind==='villain'&&(category==='minion'||catalogCategory(source)==='minion'||source.generic===true||['CAPANGA','LACAIO'].includes(String(source.tier||'').toUpperCase()));
      ...
      if(!isRepeatableMinion&&(this.state.data[key]||[]).some(item=>String(item?.sourceCharacterId||item.id)===String(id)))return;
      ...
      try{
        let removal=null;
        if(!isRepeatableMinion){
          for(const otherKey of otherKeys){const match=(this.state.data[otherKey]||[]).find(item=>String(item?.sourceCharacterId||item.id)===String(id));if(!match)continue;removal=otherKey==='heroes'?await API.deleteHero(match.id):otherKey==='npcs'?await API.deleteNpc(match.id):await API.deleteVillain(match.id);}
        }
        if(kind==='hero')await API.saveHero(entity);else if(kind==='npc')await API.saveNpc(entity);else await API.saveVillain(entity);
        this.setState(prev=>{const data={...prev.data,scenario:normalizedScenario(removal?.scenario||prev.data.scenario),initiative:removal?.initiative||prev.data.initiative,combat:removal?.combat||prev.data.combat};for(const otherKey of otherKeys)if(!isRepeatableMinion)data[otherKey]=(prev.data[otherKey]||[]).filter(x=>String(x?.sourceCharacterId||x.id)!==String(id));data[key]=[...(prev.data[key]||[]),entity];if(kind==='hero')data.playerNotes={...(removal?.playerNotes||prev.data.playerNotes||{}),[entity.id]:''};return{rosterFocusCharacterId:source.id,rosterCategory:category||prev.rosterCategory||'all',data};});
      ...
      return h('div',{className:'min-h-screen bg-[radial-gradient(circle_at_70%_20%,rgba(239,51,64,.12),transparent_35%),#080b10] p-4 text-white'},h('div',{className:'mx-auto max-w-6xl py-8 sm:py-14'},[
        ...
        s.createOpen?h(Card,{key:'createbox',className:'mt-5 overflow-hidden'},[
          ...
          h('div',{key:'body',className:'space-y-5 p-4 sm:p-5'},[
            ...
            h('div',{key:'roster'},[h(TinyLabel,{key:'l'},'3 · PERSONAGENS POR CATEGORIA'),h('p',{key:'intro',className:'mt-1 text-xs text-[#778291]'},'Defina quem será Herói, NPC, Vilão ou Capanga. NPCs são controlados pelo Mestre e não são tratados automaticamente como ameaças.'),h('div',{key:'cards',className:'mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4'},[
            ...
            rosterRole?h('div',{key:'picker',className:'mt-3 rounded-xl border border-white/10 bg-black/15 p-3'},[
              ...
              h('div',{key:'grid',className:'campaign-create-roster-scroll character-scroll-area mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3'},rosterFiltered.length?rosterFiltered.map(row=>{const item=row.item,active=selectedIds.includes(item.id),reasons=row.reasons||[];return h('button',{key:item.id,type:'button',onClick:()=>this.toggleCreationCharacter(rosterRole,item.id),className:cx('relative flex items-center gap-3 rounded-xl border p-2 text-left transition',active?'border-emerald-700/60 bg-emerald-950/20':row.score>0?'border-[#ef3340]/25 bg-[#171d27] hover:border-[#ef3340]/55':'border-white/10 bg-[#111720] hover:border-white/20')},[row.score>0&&!active?h('span',{key:'rel',className:'shrink-0 rounded-md border border-[#ef3340]/25 bg-[#ef3340]/10 px-1.5 py-0.5 text-[7px] font-black text-[#ff7b85]'},'RELACIONADO'):null,h(Portrait,{key:'p',entity:item,size:'sm'}),h('div',{key:'t',className:'min-w-0 flex-1'},[h('b',{key:'n',className:'block truncate text-xs'},item.n),h('small',{key:'r',className:'block truncate text-[9px] text-[#75808f]'},item.r||`Rank ${item.rank||4}`),reasons.length?h('span',{key:'why',className:'mt-1 block truncate text-[8px] text-[#e7959b]'},reasons.join(' · ')):item.teams?h('span',{key:'team',className:'mt-1 block truncate text-[8px] text-[#687281]'},item.teams):null]),h('span',{key:'c',className:cx('grid h-6 w-6 shrink-0 place-items-center rounded-full border text-[10px] font-black',active?'border-emerald-500 text-emerald-400':'border-white/15 text-[#667180]')},active?'✓':'+')]);}):h('div',{className:'col-span-full py-8 text-center text-xs text-[#74808f]'},'Nenhum personagem encontrado nesta categoria / grupo.'))
            ]):null
            ]),
    ...
    renderCentral(){
      ...
      if(isMaster){
        identityPanel=h(Card,{key:'actors',className:'p-4 sm:p-5'},[
          ...
          h('div',{key:'switch',className:'mt-3 inline-flex rounded-xl border border-white/10 bg-black/20 p-1'},[
            h('button',{key:'villain',type:'button',onClick:()=>this.setState({masterActorView:'villain',actorKey:'',roll:null,power:''},()=>this.ensureActor()),className:cx('rounded-lg px-4 py-2 text-[10px] font-black transition',s.masterActorView!=='npc'?'bg-[#ef3340] text-white':'text-[#85909f] hover:bg-white/[.04]')},'VILÕES'),
            h('button',{key:'npc',type:'button',onClick:()=>this.setState({masterActorView:'npc',actorKey:'',roll:null,power:''},()=>this.ensureActor()),className:cx('rounded-lg px-4 py-2 text-[10px] font-black transition',s.masterActorView==='npc'?'bg-sky-600 text-white':'text-[#85909f] hover:bg-white/[.04]')},'NPCS')
          ]),
    ...
    renderRoster(kind){
      const items=kind==='hero'?this.state.data.heroes:this.state.data.villains,isMaster=this.state.profile?.role==='master';
      return h('div',{className:'space-y-5'},[
        h('div',{key:'head',className:'flex flex-wrap items-end justify-between gap-3'},[
          h('div',{key:'t'},[h(TinyLabel,{key:'s'},kind==='hero'?'DOSSIÊS':'AMEAÇAS'),h('h1',{key:'h',className:'mt-1 text-3xl font-black'},kind==='hero'?'Heróis':'Vilões'),h('p',{key:'p',className:'mt-1 text-sm text-[#7d8795]'},isMaster?'Gerencie os personagens que pertencem a esta campanha.':'Personagens disponíveis nesta campanha.')]),
          isMaster?h(Button,{key:'add',primary:true,onClick:()=>this.setState({rosterPicker:kind,rosterCategory:kind==='hero'?'hero':'villain',rosterSearch:'',rosterGroup:''})},`+ ADICIONAR ${kind==='hero'?'HERÓI':'VILÃO'}`):null
        ]),
        ...
        ])):h(Card,{className:'p-8 text-center text-sm text-[#74808f]'},isMaster?`Nenhum ${kind==='hero'?'herói':'vilão'} nesta campanha. Use o botão Adicionar.`:'Nenhum personagem disponível.'))
      ]);
    }
    renderCampaign(){
