(() => {
  ...
  function catalogCategoryLabel(category,singular=false){
    const labels=singular?{hero:'Herói',antihero:'Anti-herói',villain:'Vilão',minion:'Capanga'}:{hero:'Heróis',antihero:'Anti-heróis',villain:'Vilões',minion:'Capangas'};
  ...
  function CharacterPickerModal({kind,items,existing,onAdd,onCreate,onClose,search='',onSearch,category='',onCategory,group='',onGroup,selectedIds=[],campaignItems=[],focusId='',templates=[]}){
    ...
    return h('div',{className:'character-picker-overlay fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-3',onMouseDown:e=>{if(e.target===e.currentTarget)onClose();}},h('div',{className:'character-picker-modal w-full max-w-6xl overflow-hidden rounded-2xl border border-white/10 bg-[#0d1118] shadow-2xl'},[
      h('div',{key:'h',className:'character-picker-header flex flex-wrap items-center justify-between gap-3 border-b border-white/10 p-4 sm:p-5'},[h('div',{key:'t'},[h(TinyLabel,{key:'l'},'CATÁLOGO INTELIGENTE'),h('h2',{key:'n',className:'mt-1 text-xl font-black'},title),h('p',{key:'p',className:'mt-1 text-xs text-[#7e8997]'},`O catálogo prioriza relações e grupos. A biblioteca é compartilhada: qualquer personagem pode entrar como herói ou vilão; aqui ele será adicionado ao lado ${kind==='hero'?'jogável':'das ameaças'}.`)]),h(Button,{key:'x',onClick:onClose,className:'px-3'},'×')]),
      ...
      activeCategory==='minion'?h('div',{key:'minionSummary',className:'character-picker-summary flex flex-wrap items-center justify-between gap-2 border-b border-white/5 bg-black/15 px-4 py-2 text-[10px] text-[#8d98a6]'},[
  ...
  class CampaignEditor extends React.Component {
    ...
    render(){
      const {campaign,heroes,villains,onSave,onCancel,onAddCategory,onEditCharacter,onRemoveCharacter,onUploadPdf,saving=false}=this.props,draft=this.state.draft;
      ...
      const all=[...(heroes||[]).map(entity=>({entity,side:'hero'})),...(villains||[]).map(entity=>({entity,side:'villain'}))];
      const roster=category=>{const items=all.filter(row=>catalogCategory(row.entity)===category);return h(Card,{className:'p-4'},[h('div',{key:'h',className:'flex items-center justify-between gap-2'},[h('div',{key:'t'},[h(TinyLabel,{key:'l'},catalogCategoryLabel(category).toUpperCase()),h('b',{key:'n',className:'mt-1 block text-sm'},`${items.length} na campanha`)]),h(Button,{key:'a',onClick:()=>onAddCategory(category),className:'min-h-9 py-1'},'+ ADICIONAR')]),h('div',{key:'l',className:'campaign-editor-roster-scroll character-scroll-area mt-3 space-y-2'},items.length?items.map(({entity,side})=>h('div',{key:`${side}-${entity.id}`,className:'flex items-center gap-3 rounded-xl border border-white/8 bg-black/15 p-2'},[h(Portrait,{key:'p',entity,size:'sm'}),h('div',{key:'n',className:'min-w-0 flex-1'},[h('b',{key:'b',className:'block truncate text-xs'},entity.n),h('small',{key:'s',className:'block truncate text-[9px] text-[#7c8795]'},`${entity.r||`Rank ${entity.rank||4}`} · ${side==='hero'?'JOGÁVEL':'AMEAÇA'}`)]),h(Button,{key:'e',onClick:()=>onEditCharacter(side,entity),className:'min-h-8 px-2 py-1'},'EDITAR'),h(Button,{key:'r',danger:true,onClick:()=>onRemoveCharacter(side,entity),className:'min-h-8 px-2 py-1'},'×')])):h('div',{className:'rounded-xl border border-dashed border-white/10 p-5 text-center text-xs text-[#75808e]'},`Nenhum ${catalogCategoryLabel(category,true).toLocaleLowerCase('pt-BR')} adicionado.`))]);};
      return h('div',{className:'space-y-4'},[
        ...
        h('div',{key:'grid',className:'grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(340px,.8fr)]'},[
          ...
          h('div',{key:'rosters',className:'grid gap-4'},[roster('hero'),roster('antihero'),roster('villain'),roster('minion')])
        ]),
  ...
  function ScenarioBuilder({scenario,heroes,villains,selected,onSelect,onMove,zoom,onZoom,onReset,onResetPiece,moveMode,onMoveMode,tool,onTool,preset,onPreset,onApplyPreset,onGenerate,onClear,onResize,onEnvironment,pieceChoice,onPieceChoice,qty,onQty,onAddPieces,onEditCell,onRemovePiece,group='terrain',onGroup,moving=false}){
    ...
    return h('div',{className:'space-y-4'},[
      ...
      h(DeferredBoard,{key:'board',scenario:current,heroes,villains,role:'master',selected,onSelect,onMove,zoom,onZoom,onReset,onResetPiece,moveMode,onMoveMode,editor:true,editorTool:tool,onEditCell,onRemovePiece,moving})
    ]);
  }
  ...
  class ArachneApp extends React.Component {
    ...
    chooseCreationBase(templateId=''){
      ...
      if(template)this.setState({createMode:'template',createTemplate:template.id,createName:template.name,createHeroIds:(template.heroes||[]).map(x=>x.id),createVillainIds:(template.villains||[]).map(x=>x.id),createRosterKind:'',createRosterSide:'',createRosterSearch:'',createRosterGroup:'',createFocusCharacterId:(template.heroes||[])[0]?.id||(template.villains||[])[0]?.id||''});
      else this.setState({createMode:'blank',createTemplate:'',createName:'Nova Campanha',createHeroIds:[],createVillainIds:[],createRosterKind:'',createRosterSide:'',createRosterSearch:'',createRosterGroup:'',createFocusCharacterId:''});
    }
    toggleCreationCharacter(kind,id){
      ...
      this.setState(prev=>{const removing=prev[key].includes(id);return{[key]:removing?prev[key].filter(x=>x!==id):[...prev[key],id],[other]:removing?prev[other]:prev[other].filter(x=>x!==id),createFocusCharacterId:removing&&prev.createFocusCharacterId===id?'':id};});
    }
    ...
    openNewCharacter(kind,category=''){
      ...
      if(category==='antihero'){entity.alignment='antihero';entity.tier='ANTI-HERÓI';entity.tags=(entity.tags||[]).filter(x=>x!=='Heroic');}
      if(category==='minion'){entity.tier='CAPANGA';entity.generic=true;entity.tags=['Villainous','Capanga'];}
    ...
    async addScenarioPieces(){
      ...
      for(let i=0;i<qty&&spots.length;i++){
        ...
        const spot=spots[cursor]||spots.find(c=>!occupied.has(scenarioKey(c.x,c.y)));if(!spot)break;cursor++;occupied.add(scenarioKey(spot.x,spot.y));const pieceKind=kind==='hero'?'hero':'enemy';
    ...
    renderLogin(){
      ...
      const rosterCategory=s.createRosterKind||'',rosterSide=s.createRosterSide||rosterSideForCategory(rosterCategory),selectedIds=rosterSide==='hero'?s.createHeroIds:s.createVillainIds;
      const rosterTerm=(s.createRosterSearch||'').trim().toLocaleLowerCase('pt-BR');
      const creationSelectedIds=allSelectedCharacterIds(s);
      const rosterGroups=rosterCategory&&INTEL?.collectGroups?INTEL.collectGroups(catalog,{categoryKey:rosterCategory}):[];
      ...
      const rosterFiltered=rankedCreation.filter(row=>!rosterCategory||catalogCategory(row.item)===rosterCategory).filter(row=>!s.createRosterGroup||(INTEL?.matchesGroup?INTEL.matchesGroup(row.item,s.createRosterGroup):true)).filter(row=>!rosterTerm||`${row.item.n||''} ${row.item.r||''} ${row.item.role||''} ${row.item.teams||''}`.toLocaleLowerCase('pt-BR').includes(rosterTerm));
      const selectedNames=(category,ids)=>catalog.filter(item=>catalogCategory(item)===category&&ids.includes(item.id));
      ...
      return h('div',{className:'min-h-screen bg-[radial-gradient(circle_at_70%_20%,rgba(239,51,64,.12),transparent_35%),#080b10] p-4 text-white'},h('div',{className:'mx-auto max-w-6xl py-8 sm:py-14'},[
        ...
        s.createOpen?h(Card,{key:'createbox',className:'mt-5 overflow-hidden'},[
          h('div',{key:'ch',className:'border-b border-white/10 p-4 sm:p-5'},[h(TinyLabel,{key:'l'},'NOVA CAMPANHA'),h('h2',{key:'h',className:'mt-1 text-xl font-black'},'Monte sua mesa'),h('p',{key:'p',className:'mt-1 text-xs leading-5 text-[#7e8997]'},'Comece vazia ou use um modelo. Depois escolha exatamente quais heróis e vilões farão parte da campanha.')]),
          h('div',{key:'body',className:'space-y-5 p-4 sm:p-5'},[
            ...
            h('div',{key:'roster'},[h(TinyLabel,{key:'l'},'3 · PERSONAGENS POR CATEGORIA'),h('p',{key:'intro',className:'mt-1 text-xs text-[#778291]'},'Separe a mesa em Heróis, Anti-heróis, Vilões e Capangas. Ao escolher alguém, o Arachne reorganiza as próximas opções por relação e grupo.'),h('div',{key:'cards',className:'mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4'},[
              ['hero','Heróis','Personagens heroicos e protagonistas.'],['antihero','Anti-heróis','Personagens ambíguos que podem atuar como protagonistas.'],['villain','Vilões','Rivais, ameaças e chefes da campanha.'],['minion','Capangas','Lacaios, agentes, soldados, drones e criaturas genéricas.']
            ].map(([cat,label,desc])=>{const allIds=creationSelectedIds,chosen=selectedNames(cat,allIds);return h('button',{key:cat,type:'button',onClick:()=>this.setState({createRosterKind:s.createRosterKind===cat?'':cat,createRosterSide:s.createRosterKind===cat?'':rosterSideForCategory(cat),createRosterSearch:'',createRosterGroup:''}),className:cx('rounded-xl border p-4 text-left transition',s.createRosterKind===cat?'border-[#ef3340] bg-[#ef3340]/8':'border-white/10 bg-[#0e141c] hover:border-white/20')},[h('div',{key:'h',className:'flex items-center justify-between gap-2'},[h('b',{key:'n'},label),h('span',{key:'c',className:cx('rounded-full border px-2 py-1 text-[10px] font-black',categoryTone(cat))},chosen.length)]),h('p',{key:'p',className:'mt-1 text-[11px] leading-4 text-[#788392]'},desc),h('div',{key:'sel',className:'mt-3 flex flex-wrap gap-1'},chosen.slice(0,4).map(x=>h('span',{key:x.id,className:'rounded-md bg-white/[.05] px-2 py-1 text-[9px]'},x.n)).concat(chosen.length>4?[h('span',{key:'more',className:'rounded-md bg-white/[.05] px-2 py-1 text-[9px]'},`+${chosen.length-4}`)]:[]))]);})),
            rosterCategory?h('div',{key:'picker',className:'mt-3 rounded-xl border border-white/10 bg-black/15 p-3'},[
              h('div',{key:'head',className:'flex flex-wrap items-center justify-between gap-2'},[h('div',{key:'t'},[h(TinyLabel,{key:'l'},`ESCOLHER ${catalogCategoryLabel(rosterCategory).toUpperCase()}`),h('span',{key:'n',className:'mt-1 block text-xs text-[#7d8795]'},`${selectedNames(rosterCategory,selectedIds).length} nesta categoria · ${selectedIds.length} no lado ${rosterSide==='hero'?'jogável':'ameaças'}`)]),h('div',{key:'tools',className:'flex w-full flex-wrap gap-2'},[h('input',{key:'search',value:s.createRosterSearch,onChange:e=>this.setState({createRosterSearch:e.target.value}),placeholder:'Buscar personagem ou equipe...',className:'min-h-10 min-w-[220px] flex-1 rounded-lg border border-white/10 bg-[#0c1118] px-3 text-sm outline-none sm:w-72'}),h('select',{key:'group',value:s.createRosterGroup||'',onChange:e=>this.setState({createRosterGroup:e.target.value}),className:'min-h-10 rounded-lg border border-white/10 bg-[#111720] px-3 text-xs font-bold'},[h('option',{key:'all',value:''},'TODOS OS GRUPOS'),...rosterGroups.map(g=>h('option',{key:g.id,value:g.id},`${g.label} (${g.count})`))])])]),
              h('div',{key:'side',className:'mt-3 flex flex-wrap items-center gap-2 rounded-xl border border-white/10 bg-black/20 p-3'},[h(TinyLabel,{key:'l'},'ENTRARÁ NA CAMPANHA COMO'),h('button',{key:'hero',type:'button',onClick:()=>this.setState({createRosterSide:'hero'}),className:cx('rounded-lg border px-3 py-2 text-[10px] font-black',rosterSide==='hero'?'border-[#ef3340] bg-[#ef3340]/10':'border-white/10 text-[#7f8997]')},'JOGÁVEL'),h('button',{key:'villain',type:'button',onClick:()=>this.setState({createRosterSide:'villain'}),className:cx('rounded-lg border px-3 py-2 text-[10px] font-black',rosterSide==='villain'?'border-[#ef3340] bg-[#ef3340]/10':'border-white/10 text-[#7f8997]')},'AMEAÇA'),h('span',{key:'h',className:'text-[9px] text-[#75808f]'},'Isso permite, por exemplo, usar um herói como antagonista ou um vilão como aliado sem mudar sua categoria natural.')]),
    ...
    renderCampaign(){
      ...
      return h('div',{className:'space-y-5'},[
        ...
        h('div',{key:'roster',className:'grid gap-4 xl:grid-cols-2'},[rosterBlock('hero'),rosterBlock('antihero'),rosterBlock('villain'),rosterBlock('minion')]),
