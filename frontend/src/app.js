        ...
        identityPanel=h(Card,{key:'actors',className:'p-4 sm:p-5'},[
          h('div',{key:'actorHead',className:'flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'},[
            ...
            h('div',{key:'switch',className:'grid w-full grid-cols-2 rounded-xl border border-white/10 bg-black/25 p-1 sm:w-auto sm:min-w-[280px]'},[
              h('button',{key:'villain',type:'button',onClick:()=>this.setState({masterActorView:'villain',actorKey:'',roll:null,power:''},()=>this.ensureActor()),className:cx('min-h-10 rounded-lg px-5 py-2 text-xs font-black tracking-wide transition',s.masterActorView!=='npc'?'bg-[#ef3340] text-white shadow-[0_8px_24px_rgba(239,51,64,.18)]':'text-[#85909f] hover:bg-white/[.04]')},'VILÕES'),
              h('button',{key:'npc',type:'button',onClick:()=>this.setState({masterActorView:'npc',actorKey:'',roll:null,power:''},()=>this.ensureActor()),className:cx('min-h-10 rounded-lg px-5 py-2 text-xs font-black tracking-wide transition',s.masterActorView==='npc'?'bg-sky-600 text-white shadow-[0_8px_24px_rgba(2,132,199,.18)]':'text-[#85909f] hover:bg-white/[.04]')},'NPCS')
            ])
          ]),
