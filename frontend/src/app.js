  ...
  class ArachneApp extends React.Component {
    ...
    renderRoster(kind){
      ...
      return h('div',{className:'space-y-5'},[
        ...
        h('div',{key:'g',className:'grid gap-3 sm:grid-cols-2 xl:grid-cols-3'},items.length?items.map(entity=>h(Card,{key:entity.id,className:'overflow-hidden'},[
    ...
    renderCampaign(){
      ...
      const rosterBlock=category=>{const items=campaignAll.filter(row=>catalogCategory(row.entity)===category);return h(Card,{className:'p-4'},[h('div',{key:'h',className:'flex items-center justify-between gap-2'},[h('div',{key:'t'},[h(TinyLabel,{key:'l'},catalogCategoryLabel(category).toUpperCase()),h('b',{key:'n',className:'mt-1 block text-sm'},`${items.length} na campanha`)]),isMaster?h(Button,{key:'a',onClick:()=>this.setState({campaignEditOpen:true}),className:'min-h-8 px-2 py-1'},'GERENCIAR'):null]),h('div',{key:'g',className:'mt-3 grid gap-2 sm:grid-cols-2'},items.length?items.map(({entity,side})=>h('button',{key:`${side}-${entity.id}`,onClick:()=>this.openSheet(entity,side),className:'flex items-center gap-3 rounded-xl border border-white/10 bg-black/15 p-2 text-left hover:border-white/20'},[h(Portrait,{key:'p',entity,size:'sm'}),h('div',{key:'t',className:'min-w-0'},[h('b',{key:'n',className:'block truncate text-xs'},entity.n),h('small',{key:'r',className:'block truncate text-[9px] text-[#74808f]'},`${entity.r||`Rank ${entity.rank||4}`} · ${side==='hero'?'JOGÁVEL':'AMEAÇA'}`)])])):h('p',{className:'py-4 text-center text-xs text-[#74808f]'},'Nenhum personagem.'))]);};
      return h('div',{className:'space-y-5'},[
